...
/**
 * React state over the advisory two-run counter and the stored run records.
 *
 ...
 * runs they already have come back together, from one record, so they cannot
...
export interface UseRunAllowance {
  ...
  remaining: number
  /** True when this app's own two-run limit is reached. */
  exhausted: boolean
...
/**
 * Track the visitor's remaining runs and the runs they have completed.
 *
 * @returns The allowance, the run records, and a way to record a new one.
 */
