---
{
  "phase_number": 4,
  "total_phases": 5,
  "phase_title": "Frontend — Planning Agent Screen",
  "phase_summary": "Build the full screen-planning UI per the finalized design mock: educational overview with quota notices, city/interests goal form with the localStorage-backed run-allowance display, plan review with per-step purpose, the explicit 'Next' advance button, progressively streamed step results with failed-step and retry-synthesis states, and the final itinerary view — replacing the Phase 1 placeholder.",
  "features": [
    {
      "id": "planning_agent_example_app",
      "role": "extended",
      "scope_note": "The complete user-facing UI lands here; the golden eval suite and end-to-end verification pass land in Phase 5."
    }
  ],
  "capabilities": [
    {
      ...
      "role": "extended",
      "scope_note": "The plan_goal_form and plan_review_execute_panel surfaces realizing this capability land here; backend behavior is complete from Phases 2–3."
    }
  ],
  "tech_stack_spec": {
    "dependencies": [
    ...
    "configurations": "No new env vars. Frontend commands run from frontend/: `npm run dev`, `npm test`, `npm run lint`, `tsc -b`, `npm run build`. Backend from Phases 1–3 must be running for manual verification."
  },
  "instructions": [
    "Reference .spec4/v4/design/mock.html throughout this phase — the screen-planning layout, its plan_overview, plan_goal_form, and plan_review_execute_panel surfaces, and the shared app shell must match the finalized mock's visual design.",
    "Replace the Phase 1 placeholder in frontend/src/apps/planning/ with the full screen composed of three surfaces per the design manifest: plan_overview (the educational explanation of the planning-agent pattern, the per-run call-limit and per-session run-limit notices with their quota-conservation rationale, and the note that planning agents in general support any number of steps), plan_goal_form (city and interests inputs → Goal), and plan_review_execute_panel (Plan, PlanSteps, StepResults, Itinerary).",
    "Align component data models with the design entities: Goal {city, interests}, Plan {steps, trimmed_note}, PlanStep {number, kind, label, purpose, query, status}, StepResult {step_number, text, failed}, Itinerary {city, morning, afternoon, evening, gap_notes}, RunAllowance {used, cap} — mirroring the backend schemas from Phase 2, not inventing parallel shapes.",
    "Implement the RunAllowance as an advisory localStorage-backed counter with cap 3 per session, displayed with remaining runs in the app header consistent with the other example apps; when exhausted, disable the submit control with an inline explanation of the limit, why it exists, and when it resets — per the specification's run-limit failure mode. The hard protection remains the server-side gate; the client never enforces quota alone.",
    "Wire plan_goal_form to POST /api/planning/plan via a TanStack Query mutation in frontend/src/api/, following the existing convention of no auto-retry on quota-spending calls (code review note). On success, render the returned Plan with each step's kind, label, purpose, and query visible, plus the trimmed_note when present — the plan must be fully visible before anything executes.",
    "Render the explicit 'Next' advance button beneath the displayed plan; only on click invoke the Phase 1 SSE hook (updated to send the real POST body) against POST /api/planning/run, decrement the local RunAllowance, and transition the panel to executing state — no executor activity may start without this click.",
    "Render each streamed step_result progressively as it arrives, in plan order, with a visible in-progress indicator on the currently executing step so the user is never without progress feedback; render failed steps distinctly with their honest 'no useful results'/failed status, and render the final itinerary event as the structured morning/afternoon/evening view with gap_notes.",
    "Handle the categorized SSE error events from Phase 3: distinguish quota exhaustion (friendly explanation, try again later) from transient step failures; on synthesis failure preserve and display the completed StepResults and offer the 'retry synthesis only' action that does not decrement the local RunAllowance.",
    "Follow existing frontend conventions (single quotes, no semicolons, Tailwind class-based light/dark theming, function components and hooks only) and keep the route lazy-loaded so no other app's chunk grows.",
    "Add Vitest + React Testing Library tests: goal form validation (empty city/interests rejected), plan renders before any run call is made, the run call fires only after the Next click, streamed step results render progressively (mock the SSE hook), failed-step and quota-exhausted states render their explanations, and the run-allowance display disables submission at the cap."
  ],
  "risk_assessment": {
    "potential_bottlenecks": "The two-phase interaction (plan first, execute only on explicit click) is easy to get subtly wrong — an AI coder may auto-start execution after planning, or let TanStack Query's default retry re-fire a quota-spending mutation. SSE state management across component re-renders (events arriving while React state updates) is another common hallucination point.",
    "mitigation_strategy": "Encode the gate in component structure: the run hook is only invoked from the Next button's onClick handler, verified by a test asserting no run request occurs before the click. Explicitly set retry: false on both the plan mutation and the run stream initiation. Keep all SSE event accumulation inside the single hook from Phase 1 (updated, not duplicated) using a reducer, so ordering is preserved regardless of render timing; reference the mock for every visual decision instead of inventing layout."
  },
  "verification": "From frontend/ run `npm test`, `npm run lint`, and `tsc -b` — all pass, including the gate test proving no run request fires before the Next click. `npm run build` succeeds with /planning as its own lazy chunk. Manual (backend running with real keys): submit 'Lisbon' + 'food and history', see the plan with per-step purposes before execution, click Next, watch step results appear progressively with visible in-progress feedback (nfr_single_model_interactions_feel_responsive__with_results_typically_appearing_within_a_few_seconds_and_never_leaving_the_user_without_progress_feedback_during_longer_multi_step_runs_), confirm the quota notices, remaining-runs display, and honest failed-step rendering are present (nfr_every_example_app_is_honest_and_transparent_about_what_the_underlying_pattern_is_doing__including_intermediate_results_and_known_limits__so_the_educational_message_survives_failures_), and confirm the screen matches .spec4/v4/design/mock.html.",
  "references": [
    {
...
# Phase 4 of 5: Frontend — Planning Agent Screen

Build the full screen-planning UI per the finalized design mock: educational overview with quota notices, city/interests goal form with the localStorage-backed run-allowance display, plan review with per-step purpose, the explicit 'Next' advance button, progressively streamed step results with failed-step and retry-synthesis states, and the final itinerary view — replacing the Phase 1 placeholder.
...
*Scope for this phase: The complete user-facing UI lands here; the golden eval suite and end-to-end verification pass land in Phase 5.*
...
*Scope for this phase: The plan_goal_form and plan_review_execute_panel surfaces realizing this capability land here; backend behavior is complete from Phases 2–3.*
...
1. Reference .spec4/v4/design/mock.html throughout this phase — the screen-planning layout, its plan_overview, plan_goal_form, and plan_review_execute_panel surfaces, and the shared app shell must match the finalized mock's visual design.
2. Replace the Phase 1 placeholder in frontend/src/apps/planning/ with the full screen composed of three surfaces per the design manifest: plan_overview (the educational explanation of the planning-agent pattern, the per-run call-limit and per-session run-limit notices with their quota-conservation rationale, and the note that planning agents in general support any number of steps), plan_goal_form (city and interests inputs → Goal), and plan_review_execute_panel (Plan, PlanSteps, StepResults, Itinerary).
3. Align component data models with the design entities: Goal {city, interests}, Plan {steps, trimmed_note}, PlanStep {number, kind, label, purpose, query, status}, StepResult {step_number, text, failed}, Itinerary {city, morning, afternoon, evening, gap_notes}, RunAllowance {used, cap} — mirroring the backend schemas from Phase 2, not inventing parallel shapes.
4. Implement the RunAllowance as an advisory localStorage-backed counter with cap 3 per session, displayed with remaining runs in the app header consistent with the other example apps; when exhausted, disable the submit control with an inline explanation of the limit, why it exists, and when it resets — per the specification's run-limit failure mode. The hard protection remains the server-side gate; the client never enforces quota alone.
5. Wire plan_goal_form to POST /api/planning/plan via a TanStack Query mutation in frontend/src/api/, following the existing convention of no auto-retry on quota-spending calls (code review note). On success, render the returned Plan with each step's kind, label, purpose, and query visible, plus the trimmed_note when present — the plan must be fully visible before anything executes.
6. Render the explicit 'Next' advance button beneath the displayed plan; only on click invoke the Phase 1 SSE hook (updated to send the real POST body) against POST /api/planning/run, decrement the local RunAllowance, and transition the panel to executing state — no executor activity may start without this click.
7. Render each streamed step_result progressively as it arrives, in plan order, with a visible in-progress indicator on the currently executing step so the user is never without progress feedback; render failed steps distinctly with their honest 'no useful results'/failed status, and render the final itinerary event as the structured morning/afternoon/evening view with gap_notes.
8. Handle the categorized SSE error events from Phase 3: distinguish quota exhaustion (friendly explanation, try again later) from transient step failures; on synthesis failure preserve and display the completed StepResults and offer the 'retry synthesis only' action that does not decrement the local RunAllowance.
9. Follow existing frontend conventions (single quotes, no semicolons, Tailwind class-based light/dark theming, function components and hooks only) and keep the route lazy-loaded so no other app's chunk grows.
10. Add Vitest + React Testing Library tests: goal form validation (empty city/interests rejected), plan renders before any run call is made, the run call fires only after the Next click, streamed step results render progressively (mock the SSE hook), failed-step and quota-exhausted states render their explanations, and the run-allowance display disables submission at the cap.

## Risk Assessment

**Potential bottlenecks:**

The two-phase interaction (plan first, execute only on explicit click) is easy to get subtly wrong — an AI coder may auto-start execution after planning, or let TanStack Query's default retry re-fire a quota-spending mutation. SSE state management across component re-renders (events arriving while React state updates) is another common hallucination point.

**Mitigation strategy:**

Encode the gate in component structure: the run hook is only invoked from the Next button's onClick handler, verified by a test asserting no run request occurs before the click. Explicitly set retry: false on both the plan mutation and the run stream initiation. Keep all SSE event accumulation inside the single hook from Phase 1 (updated, not duplicated) using a reducer, so ordering is preserved regardless of render timing; reference the mock for every visual decision instead of inventing layout.
