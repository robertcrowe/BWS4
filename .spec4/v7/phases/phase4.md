---
{
  ...
  "phase_summary": "Build the visitor-facing ReAct Loop screen against the finalized mock: a preset selector and a start control that shows no plan and asks for no mid-run approval, a trace that fills cycle by cycle as the SSE envelopes arrive, a live 'search N of 8' counter, two visually distinct terminal cards, and the two-run session allowance whose exhausted state disables input while leaving every previous trace on screen.",
  "features": [
    {
  ...
  "tech_stack_spec": {
    ...
    "configurations": "The frontend calls the api target over HTTPS only; the api allows only the web_client's own origin via CORS_ORIGIN. VITE_SENTRY_DSN is optional and Sentry is never initialized when unset. The two-run session counter and prior run records are held in browser localStorage under a react_run_allowance key, stamped with the UTC hour so it resets on the same clock as the server-side gate."
  },
  "instructions": [
    "Reference .spec4/v7/design/mock.html throughout this phase and match the ReAct Loop screen's visual design: the trace's thought/action/observation blocks, the live cycle counter, the runs-remaining indicator, and the two terminal cards. The mock is the authority on layout, spacing, colour and light/dark treatment.",
    "BUDGET DECISION — AUTHORITATIVE: the counter reads 'search N of 8' against a server-fixed budget. There is NO visitor-facing budget control and NO 3..6 clamp; the attached react_search_loop specification's visitor-supplied `cycle_budget` is SUPERSEDED. Read the budget from the run_started envelope's cycle_budget field rather than hard-coding 8 in the component, so the display always mirrors the server.",
    "Build the screen in frontend/src/apps/react/, following the folder and component conventions of the existing frontend/src/apps/planning/ and frontend/src/apps/orchestrated/ modules. Use the shared layout shell, nav bar and theme toggle from frontend/src/components/ so this app matches every other example app's layout without relearning.",
    "Every new TypeScript file opens with `// Built with Spec4 AI - https://spec4.ai` and every exported function carries JSDoc. Use function components and hooks only — no class components.",
    "Build the preset selector from GET /api/react/presets, showing each of the five curated questions. Presets carry questions only — the UI must never display or expect a stored answer.",
    "Build the start control so that pressing it begins the run immediately: NO plan is displayed before the run and NO approval is requested mid-run. This is the precise contrast with the Planning Agent example and it must be structurally true of the UI, not merely described in copy.",
    "Consume POST /api/react/run with @microsoft/fetch-event-source in a dedicated hook under frontend/src/api/, following the SSE hooks already written for the planning, orchestrated and collab runs. The browser's native EventSource is GET-only and cannot be used because the run starts from a POST body.",
    "Wire the hook's abort to component unmount and to a visible stop control, so navigating away aborts the stream — an abandoned run must stop spending quota, which matters most for this, the gallery's most expensive example.",
    "Render each cycle progressively AS ITS ENVELOPES ARRIVE — never buffer the run and render at the end. A cycle_thought appears, then its cycle_action showing either the exact query issued or the decision to answer, then its cycle_observation showing the verbatim snippets with their titles and sources. The next thought must be visibly able to build on the observation above it.",
    "Render observation snippets as ESCAPED TEXT with no HTML or script execution and no auto-followed links, clearly attributed with source title and URL. These are untrusted third-party web results. Use the shared react-markdown wrapper in frontend/src/components/ for model-produced prose — never dangerouslySetInnerHTML on this unauthenticated public surface.",
    "Render an empty observation explicitly as a visible 'no results' state, and a failed search as a visible 'search unavailable' state. Neither may be hidden or rendered as a blank cycle.",
    "Render the live cycle counter from the cycle_counter envelopes, advancing as the run proceeds so the visitor can watch the budget being consumed and anticipate a budget-exhausted ending.",
    "Render the two terminal cards as VISUALLY DISTINCT states following the mock: a final-answer card naming which observations the answer drew on (and surfacing the grounding audit's unverified indices where any exist), and a budget-exhausted card presenting the partial trace and naming what remained unresolved. The budget-exhausted card must never be styled or worded as though it were an answer.",
    "Implement the two-run-per-visit session allowance in browser localStorage under a react_run_allowance key holding the counter, the UTC hour stamp, and each prior run's run_id and rendered trace. Stamping with the UTC hour makes the counter reset on the same clock as the server-side showcase-wide gate.",
    "Show a visible runs-remaining indicator at all times, mirroring the treatment used by the orchestrated-subagents app.",
    "When the two runs are exhausted, DISABLE the question input and the start control with a clear explanatory message, and LEAVE ALL PREVIOUS RESULTS ON SCREEN. The message must distinguish plainly between hitting THIS app's two-run session limit and hitting the shared framework-wide usage cap — these are different situations and the visitor must be told which one they hit.",
    "Handle a 429 from the server by rendering the server's own limit message, again distinguishing this app's limit from the shared cap, and leaving prior results on screen.",
    "Re-fetch a prior run's full trace from GET /api/react/run/{run_id} using the stored run_id rather than trusting the cached localStorage copy, so a restored trace is always the authoritative one.",
    "Make the screen responsive across common desktop and mobile widths using Tailwind, and readable by assistive technologies: give the trace region an appropriate live-region role so cycles are announced as they arrive, label the counter and runs-remaining indicator, and ensure the two terminal cards are distinguishable by more than colour alone.",
    "Add Vitest + React Testing Library tests in frontend/tests/: the trace renders cycles in arrival order; the cycle counter advances as cycle_counter envelopes arrive; the budget-exhausted card is distinguishable from the final-answer card; an empty observation renders a visible no-results state; exhausting the two-run allowance disables the input and start control while previous traces remain on screen; and the exhausted message text distinguishes the session limit from the framework cap. Drive these with a mocked SSE envelope sequence, not a live backend.",
    "Do not modify any other example app's code in this phase."
  ],
  "risk_assessment": {
    "potential_bottlenecks": "The most likely implementation error is buffering: an AI coder writing an SSE consumer that accumulates envelopes and sets state once at stream close produces a screen that works perfectly in tests and destroys the entire point of the exhibit, since the loop's visible progression IS the lesson. React 19 batching can also make several rapidly-arriving envelopes render as one jump. A second risk is the exhausted state being implemented as a clear-and-replace, wiping previous traces exactly when the spec requires them to stay. A third is the localStorage allowance being mistaken for real quota protection — it is advisory only and resettable by the visitor; hard protection is always the server-side gate and the reservation. Finally, snippet rendering is an XSS surface: these are untrusted third-party web results on an unauthenticated public page.",
    "mitigation_strategy": "For buffering, the instruction states explicitly that each envelope updates state as it arrives, and the Vitest suite feeds envelopes one at a time asserting the DOM grows between them — a buffered implementation fails that test. For the exhausted state, it is implemented as an additive disabled overlay on the controls rather than a state transition that unmounts the results region, and a test asserts prior traces are still in the document after exhaustion. The advisory nature of the localStorage counter is stated in the instructions, and Phase 3 already made the server-side reservation the real gate, so a cleared localStorage cannot exceed the shared allowance. For XSS, snippets render as escaped text and model prose goes through the existing shared react-markdown wrapper, matching the pattern the rest of the gallery already uses."
  },
  "verification": "Run `npm --prefix frontend run test` — all new Vitest suites green, including the assertion that the DOM grows between individually-delivered envelopes (proving no buffering), that the counter advances, that the two terminal cards are distinguishable, and that exhausting the allowance disables input while leaving prior traces on screen. Run `npm --prefix frontend run lint` and `npm --prefix frontend run build` — clean, with the ReAct route emitted as its own lazy chunk. Then run the app against the live backend with `npm --prefix frontend run dev` and `uv run uvicorn backend.app.main:app --reload`: select preset p1, press start, and confirm no plan is shown, no approval is requested, cycles appear one at a time with the counter advancing, snippets are attributed and rendered as inert text, and the run ends in exactly one terminal card. Run twice more and confirm the third attempt is blocked with a message that names this app's two-run limit as distinct from the shared framework cap, with both prior traces still visible. Compare the rendered screen against .spec4/v7/design/mock.html. Goal checks: nfr_every_example_app_follows_the_same_overall_layout_and_navigation__so_a_visitor_who_understands_one_can_immediately_use_the_next (shared layout shell reused, verified against the mock); nfr_the_gallery_is_usable_on_common_desktop_and_mobile_screen_sizes_and_readable_by_assistive_technologies (responsive at mobile and desktop widths; trace announced via live region; terminal cards distinguishable beyond colour); nfr_static_content_and_plots_appear_within_about_a_second__runs_that_involve_model_work_show_progress_immediately_and_reveal_intermediate_results_as_they_complete_rather_than_waiting_for_the_end (page renders before any model work; cycles appear as they complete); nfr_when_any_usage_limit_is_reached__the_visitor_is_told_plainly_which_limit_it_was_and_any_results_already_produced_remain_on_screen; nfr_the_gallery_is_free_to_visit_and_requires_no_sign_up_or_personal_information (the allowance is a device-local counter with no account and no personal data).",
  "references": [
    {
...
# Phase 4 of 8: The ReAct Screen — Live Trace, Cycle Counter, Runs-Remaining and Exhausted State

Build the visitor-facing ReAct Loop screen against the finalized mock: a preset selector and a start control that shows no plan and asks for no mid-run approval, a trace that fills cycle by cycle as the SSE envelopes arrive, a live 'search N of 8' counter, two visually distinct terminal cards, and the two-run session allowance whose exhausted state disables input while leaving every previous trace on screen.
...
*Scope for this phase: The preset-driven UI lands here — trace stream, cycle counter, terminal cards, runs-remaining and the exhausted state; the free-form question input's moderation and suitability advisory land in Phase 5, the hop-annotation panel in Phase 6, and the educational overview copy in Phase 7.*
...
*Scope for this phase: The client-side realization of the loop — consuming the per-cycle SSE envelopes, rendering thought/action/observation in arrival order, and the two-run per-visit allowance; the backend loop was completed in Phase 3.*
...
1. Reference .spec4/v7/design/mock.html throughout this phase and match the ReAct Loop screen's visual design: the trace's thought/action/observation blocks, the live cycle counter, the runs-remaining indicator, and the two terminal cards. The mock is the authority on layout, spacing, colour and light/dark treatment.
2. BUDGET DECISION — AUTHORITATIVE: the counter reads 'search N of 8' against a server-fixed budget. There is NO visitor-facing budget control and NO 3..6 clamp; the attached react_search_loop specification's visitor-supplied `cycle_budget` is SUPERSEDED. Read the budget from the run_started envelope's cycle_budget field rather than hard-coding 8 in the component, so the display always mirrors the server.
3. Build the screen in frontend/src/apps/react/, following the folder and component conventions of the existing frontend/src/apps/planning/ and frontend/src/apps/orchestrated/ modules. Use the shared layout shell, nav bar and theme toggle from frontend/src/components/ so this app matches every other example app's layout without relearning.
4. Every new TypeScript file opens with `// Built with Spec4 AI - https://spec4.ai` and every exported function carries JSDoc. Use function components and hooks only — no class components.
5. Build the preset selector from GET /api/react/presets, showing each of the five curated questions. Presets carry questions only — the UI must never display or expect a stored answer.
6. Build the start control so that pressing it begins the run immediately: NO plan is displayed before the run and NO approval is requested mid-run. This is the precise contrast with the Planning Agent example and it must be structurally true of the UI, not merely described in copy.
7. Consume POST /api/react/run with @microsoft/fetch-event-source in a dedicated hook under frontend/src/api/, following the SSE hooks already written for the planning, orchestrated and collab runs. The browser's native EventSource is GET-only and cannot be used because the run starts from a POST body.
8. Wire the hook's abort to component unmount and to a visible stop control, so navigating away aborts the stream — an abandoned run must stop spending quota, which matters most for this, the gallery's most expensive example.
9. Render each cycle progressively AS ITS ENVELOPES ARRIVE — never buffer the run and render at the end. A cycle_thought appears, then its cycle_action showing either the exact query issued or the decision to answer, then its cycle_observation showing the verbatim snippets with their titles and sources. The next thought must be visibly able to build on the observation above it.
10. Render observation snippets as ESCAPED TEXT with no HTML or script execution and no auto-followed links, clearly attributed with source title and URL. These are untrusted third-party web results. Use the shared react-markdown wrapper in frontend/src/components/ for model-produced prose — never dangerouslySetInnerHTML on this unauthenticated public surface.
11. Render an empty observation explicitly as a visible 'no results' state, and a failed search as a visible 'search unavailable' state. Neither may be hidden or rendered as a blank cycle.
12. Render the live cycle counter from the cycle_counter envelopes, advancing as the run proceeds so the visitor can watch the budget being consumed and anticipate a budget-exhausted ending.
13. Render the two terminal cards as VISUALLY DISTINCT states following the mock: a final-answer card naming which observations the answer drew on (and surfacing the grounding audit's unverified indices where any exist), and a budget-exhausted card presenting the partial trace and naming what remained unresolved. The budget-exhausted card must never be styled or worded as though it were an answer.
14. Implement the two-run-per-visit session allowance in browser localStorage under a react_run_allowance key holding the counter, the UTC hour stamp, and each prior run's run_id and rendered trace. Stamping with the UTC hour makes the counter reset on the same clock as the server-side showcase-wide gate.
15. Show a visible runs-remaining indicator at all times, mirroring the treatment used by the orchestrated-subagents app.
16. When the two runs are exhausted, DISABLE the question input and the start control with a clear explanatory message, and LEAVE ALL PREVIOUS RESULTS ON SCREEN. The message must distinguish plainly between hitting THIS app's two-run session limit and hitting the shared framework-wide usage cap — these are different situations and the visitor must be told which one they hit.
17. Handle a 429 from the server by rendering the server's own limit message, again distinguishing this app's limit from the shared cap, and leaving prior results on screen.
18. Re-fetch a prior run's full trace from GET /api/react/run/{run_id} using the stored run_id rather than trusting the cached localStorage copy, so a restored trace is always the authoritative one.
19. Make the screen responsive across common desktop and mobile widths using Tailwind, and readable by assistive technologies: give the trace region an appropriate live-region role so cycles are announced as they arrive, label the counter and runs-remaining indicator, and ensure the two terminal cards are distinguishable by more than colour alone.
20. Add Vitest + React Testing Library tests in frontend/tests/: the trace renders cycles in arrival order; the cycle counter advances as cycle_counter envelopes arrive; the budget-exhausted card is distinguishable from the final-answer card; an empty observation renders a visible no-results state; exhausting the two-run allowance disables the input and start control while previous traces remain on screen; and the exhausted message text distinguishes the session limit from the framework cap. Drive these with a mocked SSE envelope sequence, not a live backend.
21. Do not modify any other example app's code in this phase.

## Risk Assessment

**Potential bottlenecks:**

The most likely implementation error is buffering: an AI coder writing an SSE consumer that accumulates envelopes and sets state once at stream close produces a screen that works perfectly in tests and destroys the entire point of the exhibit, since the loop's visible progression IS the lesson. React 19 batching can also make several rapidly-arriving envelopes render as one jump. A second risk is the exhausted state being implemented as a clear-and-replace, wiping previous traces exactly when the spec requires them to stay. A third is the localStorage allowance being mistaken for real quota protection — it is advisory only and resettable by the visitor; hard protection is always the server-side gate and the reservation. Finally, snippet rendering is an XSS surface: these are untrusted third-party web results on an unauthenticated public page.

**Mitigation strategy:**

For buffering, the instruction states explicitly that each envelope updates state as it arrives, and the Vitest suite feeds envelopes one at a time asserting the DOM grows between them — a buffered implementation fails that test. For the exhausted state, it is implemented as an additive disabled overlay on the controls rather than a state transition that unmounts the results region, and a test asserts prior traces are still in the document after exhaustion. The advisory nature of the localStorage counter is stated in the instructions, and Phase 3 already made the server-side reservation the real gate, so a cleared localStorage cannot exceed the shared allowance. For XSS, snippets render as escaped text and model prose goes through the existing shared react-markdown wrapper, matching the pattern the rest of the gallery already uses.

## Verification

Run `npm --prefix frontend run test` — all new Vitest suites green, including the assertion that the DOM grows between individually-delivered envelopes (proving no buffering), that the counter advances, that the two terminal cards are distinguishable, and that exhausting the allowance disables input while leaving prior traces on screen. Run `npm --prefix frontend run lint` and `npm --prefix frontend run build` — clean, with the ReAct route emitted as its own lazy chunk. Then run the app against the live backend with `npm --prefix frontend run dev` and `uv run uvicorn backend.app.main:app --reload`: select preset p1, press start, and confirm no plan is shown, no approval is requested, cycles appear one at a time with the counter advancing, snippets are attributed and rendered as inert text, and the run ends in exactly one terminal card. Run twice more and confirm the third attempt is blocked with a message that names this app's two-run limit as distinct from the shared framework cap, with both prior traces still visible. Compare the rendered screen against .spec4/v7/design/mock.html. Goal checks: nfr_every_example_app_follows_the_same_overall_layout_and_navigation__so_a_visitor_who_understands_one_can_immediately_use_the_next (shared layout shell reused, verified against the mock); nfr_the_gallery_is_usable_on_common_desktop_and_mobile_screen_sizes_and_readable_by_assistive_technologies (responsive at mobile and desktop widths; trace announced via live region; terminal cards distinguishable beyond colour); nfr_static_content_and_plots_appear_within_about_a_second__runs_that_involve_model_work_show_progress_immediately_and_reveal_intermediate_results_as_they_complete_rather_than_waiting_for_the_end (page renders before any model work; cycles appear as they complete); nfr_when_any_usage_limit_is_reached__the_visitor_is_told_plainly_which_limit_it_was_and_any_results_already_produced_remain_on_screen; nfr_the_gallery_is_free_to_visit_and_requires_no_sign_up_or_personal_information (the allowance is a device-local counter with no account and no personal data).
