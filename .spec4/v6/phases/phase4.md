---
{
  "phase_number": 4,
  "total_phases": 6,
  "phase_title": "Negotiation Run UI — Scenario Form, Buyer Track, Parallel Seller Columns, and the Message Log",
  "phase_summary": "Build the browser experience for the negotiation: a closed-choice scenario and priority-weighting form with no free-text input, an SSE-consuming stage rail that renders the buyer track and two live seller columns updating independently as the concurrent bids land, a term-by-term diff table across the four axes, and a toggleable chronological sender-to-recipient message log whose missing seller-to-seller lane makes the opacity claim visible at a glance.",
  "features": [
    {
      "id": "multi_agent_collaboration_example_app",
      "role": "extended",
      "scope_note": "The collab_scenario_form, collab_negotiation_run and collab_message_log surfaces land here; the reveal and priority-sensitivity panels are Phase 5."
    }
  ],
  "capabilities": [
    {
      ...
      "role": "extended",
      "scope_note": "Adds the browser-side views onto the six negotiation stages streamed by Phase 3; the post-award explanation panels are Phase 5."
    }
  ],
  "tech_stack_spec": {
    "dependencies": [
    ...
    "configurations": "The frontend calls the API origin already configured for the existing examples; the API's CORS remains pinned to the single web_client origin and the SPA is served over HTTPS only. VITE_SENTRY_DSN remains optional. No new environment variables. Do not use the browser's native EventSource — it is GET-only and this run starts from a POST payload."
  },
  "instructions": [
    "Build the collab_scenario_form surface in `frontend/src/apps/collab/`: a preset scenario selector over the fixed catalog and a priority-weighting control over the four axes (price, delivery speed, quantity, warranty). There is deliberately no free-text input on this screen — do not add one, and do not add a moderation affordance, because closed enums and a numeric vector are what make this example injection-proof.",
    "Display the run's declared cost next to the start control before the run begins: 8 model calls, stated as 6 negotiation calls plus 2 post-award explanation calls. Also state that runs are limited by the framework's standard shared hourly and daily allowance — this app has no tightened per-app session counter of its own.",
    "Add an SSE consumption hook in `frontend/src/api/` using `@microsoft/fetch-event-source`, following the pattern the existing planning and orchestrated hooks use, since the run starts from a POST body that the native EventSource cannot send. Handle abort on unmount so navigating away stops the stream, and surface connection errors distinctly from cap refusals.",
    "Build the collab_negotiation_run surface with a persistent stage rail numbered 1 through 6 that shows which stage is active, complete or failed, so the dense flow stays followable.",
    "Render the buyer track as a sequential lane carrying the QuotationRequest, the two counter-offers, and the award with its rationale and per-priority references. Render the two sellers as parallel columns with live independent per-seller status, so each column visibly progresses on its own as its concurrent bid completes — matching the treatment the orchestrated-subagents specialist columns already use, so a visitor who learned that screen needs no relearning.",
    "Build the term-by-term diff table collapsing each seller's movement across the four axes from opening bid to best-and-final, so the trade-off the buyer had to weigh is legible without reading prose.",
    "Render all model-produced prose — the award rationale, bid notes, counter-offer justifications — through the shared react-markdown wrapper in `frontend/src/components/`, never via dangerouslySetInnerHTML, since this is an unauthenticated public surface. Do not add remark-gfm; it is roadmap-only for this build.",
    "Build the collab_message_log surface in `frontend/src/apps/collab/message-log.tsx` as a hand-rolled Tailwind-styled table, toggleable and collapsed by default rather than always shown. One row per envelope: sequence, time, sender to recipient, stage, and a summary, with expandable pretty-printed JSON work items. Keep it deliberately dense and chronological so the absence of any seller-to-seller row is visible at a glance. Add no JSON-viewer dependency, no syntax-highlighting library and no copy-to-clipboard.",
    "Handle the degraded and failure states the backend emits: a failing seller column shows its own error state while the other column's bid remains on screen; a run halted at a buyer stage shows the partial record with an explanatory banner and a retry affordance; an award that failed reconciliation shows the 'rationale did not reconcile with weights' banner rather than presenting the rationale as sound.",
    "Handle the cap-refusal state distinctly from an upstream service problem: when the shared allowance refuses the run up front, show a specific message naming the shared framework limit and its reset, keep any previously produced results on screen, and disable the start control with that explanation rather than failing silently on click.",
    "Reference `.spec4/v6/design/mock.html` throughout for the visual design of the scenario form, buyer track, parallel seller columns, diff table and message-log table, and keep the shared layout shell, nav bar and light/dark theme treatment consistent with the other example screens.",
    "Open every new TypeScript file with `// Built with Spec4 AI - https://spec4.ai` and add JSDoc to every exported function, per project convention.",
    "Write Vitest + React Testing Library tests against a mocked SSE stream: that both seller columns render an in-progress state simultaneously when neither bid event has arrived; that one seller's bid event updates only its own column and leaves the other in progress; that a seller error event leaves the other column's bid rendered; that the message log renders every delivered envelope in sequence order and contains no row whose sender and recipient are both sellers; that the declared 8-call cost is shown before the run starts; and that a cap-refusal response disables the start control with the shared-limit explanation while prior results remain visible.",
    "Run `cd frontend && npm run lint` with oxlint and `cd frontend && npx tsc -b` and keep both clean for the new files."
  ],
  "risk_assessment": {
    "potential_bottlenecks": "The parallelism is the point of this screen and it is easy to lose: if both seller columns are rendered from a single state object updated on each event, or if the events are buffered and applied together, the columns will appear to complete simultaneously and the visible fan-out — the thing the demo teaches — silently disappears even though the backend really did run them concurrently. Second, SSE over POST is a known trap: an AI coder reaching for the native EventSource will find it cannot send the body and may invent a GET endpoint with query parameters, forking the API. Third, Render's free-tier spin-down means a cold start can precede the first event by many seconds, which reads as a hung UI. Fourth, this screen has many states — six stages, two independent tracks, degraded, halted, unreconciled, cap-refused — and an incomplete state machine tends to render blank panels.",
    "mitigation_strategy": "Hold per-seller state in independent slices keyed by seller id and apply each event to only its own slice as it arrives, and prove it with the test asserting one bid event leaves the sibling column in progress — that test is the real guard on the teaching point. Use @microsoft/fetch-event-source exclusively and follow the existing planning/orchestrated hooks rather than inventing a transport; do not add a GET variant of the run endpoint. Render an explicit connecting state before the first event and rely on sse-starlette's keep-alive pings from Phase 3, treating cold start as a routine path rather than an edge case. Enumerate the state machine explicitly in one reducer with a named state per condition so no combination falls through to an empty render, and cover the degraded, halted, unreconciled and cap-refused branches in tests."
  },
  "verification": "Run `cd frontend && npm run test` — all seller-column independence, message-log, cost-disclosure and cap-refusal tests pass. Run `cd frontend && npm run build` and confirm a clean build with the collab route in its own chunk. With the API running, open /collab in the browser, select a preset scenario and weighting, and start a run: confirm the stated 8-call cost appears before starting; confirm both seller columns show live independent progress and complete at different moments, verifying nfr_multi_step_runs_reveal_results_progressively_as_each_step_completes_rather_than_only_at_the_end__so_waiting_is_informative_; confirm the buyer track shows the RFQ, counter-offers and award, and the message log toggle reveals a chronological sender-to-recipient table with no seller-to-seller row, verifying nfr_intermediate_steps___retrieved_passages__plans__delegation_decisions__per_agent_messages___are_visible_to_the_visitor_rather_than_hidden__since_making_agent_behaviour_observable_is_the_point_of_the_project_. Confirm the screen matches .spec4/v6/design/mock.html and shares the layout and theme of the orchestrated screen, verifying nfr_all_examples_share_one_consistent_page_layout__so_a_visitor_who_learns_one_can_navigate_the_rest_without_relearning_ and, at 1280px width, nfr_the_examples_remain_usable_on_a_laptop_sized_screen_with_a_typical_consumer_network_connection_. Exhaust the shared allowance and confirm the start control disables with a shared-limit message distinct from a service error while prior results stay on screen, verifying nfr_when_a_limit_is_reached_or_an_upstream_capability_is_unavailable__the_visitor_sees_a_clear__specific_explanation_that_distinguishes_the_two__and_any_results_already_produced_remain_on_screen_. Confirm no sign-up or personal information is requested at any point, verifying nfr_no_sign_up__personal_information_or_payment_is_required_to_use_any_example_.",
  "references": [
    {
...
# Phase 4 of 6: Negotiation Run UI — Scenario Form, Buyer Track, Parallel Seller Columns, and the Message Log

Build the browser experience for the negotiation: a closed-choice scenario and priority-weighting form with no free-text input, an SSE-consuming stage rail that renders the buyer track and two live seller columns updating independently as the concurrent bids land, a term-by-term diff table across the four axes, and a toggleable chronological sender-to-recipient message log whose missing seller-to-seller lane makes the opacity claim visible at a glance.
...
*Scope for this phase: The collab_scenario_form, collab_negotiation_run and collab_message_log surfaces land here; the reveal and priority-sensitivity panels are Phase 5.*
...
*Scope for this phase: Adds the browser-side views onto the six negotiation stages streamed by Phase 3; the post-award explanation panels are Phase 5.*
...
1. Build the collab_scenario_form surface in `frontend/src/apps/collab/`: a preset scenario selector over the fixed catalog and a priority-weighting control over the four axes (price, delivery speed, quantity, warranty). There is deliberately no free-text input on this screen — do not add one, and do not add a moderation affordance, because closed enums and a numeric vector are what make this example injection-proof.
2. Display the run's declared cost next to the start control before the run begins: 8 model calls, stated as 6 negotiation calls plus 2 post-award explanation calls. Also state that runs are limited by the framework's standard shared hourly and daily allowance — this app has no tightened per-app session counter of its own.
3. Add an SSE consumption hook in `frontend/src/api/` using `@microsoft/fetch-event-source`, following the pattern the existing planning and orchestrated hooks use, since the run starts from a POST body that the native EventSource cannot send. Handle abort on unmount so navigating away stops the stream, and surface connection errors distinctly from cap refusals.
4. Build the collab_negotiation_run surface with a persistent stage rail numbered 1 through 6 that shows which stage is active, complete or failed, so the dense flow stays followable.
5. Render the buyer track as a sequential lane carrying the QuotationRequest, the two counter-offers, and the award with its rationale and per-priority references. Render the two sellers as parallel columns with live independent per-seller status, so each column visibly progresses on its own as its concurrent bid completes — matching the treatment the orchestrated-subagents specialist columns already use, so a visitor who learned that screen needs no relearning.
6. Build the term-by-term diff table collapsing each seller's movement across the four axes from opening bid to best-and-final, so the trade-off the buyer had to weigh is legible without reading prose.
7. Render all model-produced prose — the award rationale, bid notes, counter-offer justifications — through the shared react-markdown wrapper in `frontend/src/components/`, never via dangerouslySetInnerHTML, since this is an unauthenticated public surface. Do not add remark-gfm; it is roadmap-only for this build.
8. Build the collab_message_log surface in `frontend/src/apps/collab/message-log.tsx` as a hand-rolled Tailwind-styled table, toggleable and collapsed by default rather than always shown. One row per envelope: sequence, time, sender to recipient, stage, and a summary, with expandable pretty-printed JSON work items. Keep it deliberately dense and chronological so the absence of any seller-to-seller row is visible at a glance. Add no JSON-viewer dependency, no syntax-highlighting library and no copy-to-clipboard.
9. Handle the degraded and failure states the backend emits: a failing seller column shows its own error state while the other column's bid remains on screen; a run halted at a buyer stage shows the partial record with an explanatory banner and a retry affordance; an award that failed reconciliation shows the 'rationale did not reconcile with weights' banner rather than presenting the rationale as sound.
10. Handle the cap-refusal state distinctly from an upstream service problem: when the shared allowance refuses the run up front, show a specific message naming the shared framework limit and its reset, keep any previously produced results on screen, and disable the start control with that explanation rather than failing silently on click.
11. Reference `.spec4/v6/design/mock.html` throughout for the visual design of the scenario form, buyer track, parallel seller columns, diff table and message-log table, and keep the shared layout shell, nav bar and light/dark theme treatment consistent with the other example screens.
12. Open every new TypeScript file with `// Built with Spec4 AI - https://spec4.ai` and add JSDoc to every exported function, per project convention.
13. Write Vitest + React Testing Library tests against a mocked SSE stream: that both seller columns render an in-progress state simultaneously when neither bid event has arrived; that one seller's bid event updates only its own column and leaves the other in progress; that a seller error event leaves the other column's bid rendered; that the message log renders every delivered envelope in sequence order and contains no row whose sender and recipient are both sellers; that the declared 8-call cost is shown before the run starts; and that a cap-refusal response disables the start control with the shared-limit explanation while prior results remain visible.
14. Run `cd frontend && npm run lint` with oxlint and `cd frontend && npx tsc -b` and keep both clean for the new files.

## Risk Assessment

**Potential bottlenecks:**

The parallelism is the point of this screen and it is easy to lose: if both seller columns are rendered from a single state object updated on each event, or if the events are buffered and applied together, the columns will appear to complete simultaneously and the visible fan-out — the thing the demo teaches — silently disappears even though the backend really did run them concurrently. Second, SSE over POST is a known trap: an AI coder reaching for the native EventSource will find it cannot send the body and may invent a GET endpoint with query parameters, forking the API. Third, Render's free-tier spin-down means a cold start can precede the first event by many seconds, which reads as a hung UI. Fourth, this screen has many states — six stages, two independent tracks, degraded, halted, unreconciled, cap-refused — and an incomplete state machine tends to render blank panels.

**Mitigation strategy:**

Hold per-seller state in independent slices keyed by seller id and apply each event to only its own slice as it arrives, and prove it with the test asserting one bid event leaves the sibling column in progress — that test is the real guard on the teaching point. Use @microsoft/fetch-event-source exclusively and follow the existing planning/orchestrated hooks rather than inventing a transport; do not add a GET variant of the run endpoint. Render an explicit connecting state before the first event and rely on sse-starlette's keep-alive pings from Phase 3, treating cold start as a routine path rather than an edge case. Enumerate the state machine explicitly in one reducer with a named state per condition so no combination falls through to an empty render, and cover the degraded, halted, unreconciled and cap-refused branches in tests.

## Verification

Run `cd frontend && npm run test` — all seller-column independence, message-log, cost-disclosure and cap-refusal tests pass. Run `cd frontend && npm run build` and confirm a clean build with the collab route in its own chunk. With the API running, open /collab in the browser, select a preset scenario and weighting, and start a run: confirm the stated 8-call cost appears before starting; confirm both seller columns show live independent progress and complete at different moments, verifying nfr_multi_step_runs_reveal_results_progressively_as_each_step_completes_rather_than_only_at_the_end__so_waiting_is_informative_; confirm the buyer track shows the RFQ, counter-offers and award, and the message log toggle reveals a chronological sender-to-recipient table with no seller-to-seller row, verifying nfr_intermediate_steps___retrieved_passages__plans__delegation_decisions__per_agent_messages___are_visible_to_the_visitor_rather_than_hidden__since_making_agent_behaviour_observable_is_the_point_of_the_project_. Confirm the screen matches .spec4/v6/design/mock.html and shares the layout and theme of the orchestrated screen, verifying nfr_all_examples_share_one_consistent_page_layout__so_a_visitor_who_learns_one_can_navigate_the_rest_without_relearning_ and, at 1280px width, nfr_the_examples_remain_usable_on_a_laptop_sized_screen_with_a_typical_consumer_network_connection_. Exhaust the shared allowance and confirm the start control disables with a shared-limit message distinct from a service error while prior results stay on screen, verifying nfr_when_a_limit_is_reached_or_an_upstream_capability_is_unavailable__the_visitor_sees_a_clear__specific_explanation_that_distinguishes_the_two__and_any_results_already_produced_remain_on_screen_. Confirm no sign-up or personal information is requested at any point, verifying nfr_no_sign_up__personal_information_or_payment_is_required_to_use_any_example_.
