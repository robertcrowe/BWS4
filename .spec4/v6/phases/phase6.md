---
{
  "phase_number": 6,
  "total_phases": 6,
  "phase_title": "Eval Suite, Opacity Proof, Telemetry and Deploy Readiness",
  "phase_summary": "Prove the claims the example makes. Land the golden-run and adversarial-probe suites, assert the negotiation-stage count is exactly six and the total exactly eight, prove seller-to-seller traffic is zero by SQL across every preset, demonstrate that the same scenario flips winner under different weightings, wire the structlog telemetry the stack specifies, and confirm the slice deploys cleanly on Render's free tier including the cold-start path.",
  "features": [
    {
      "id": "multi_agent_collaboration_example_app",
      "role": "extended",
      "scope_note": "No new user-facing surface — this phase verifies, instruments and hardens the feature delivered in Phases 1-5."
    }
  ],
  "capabilities": [
    {
      ...
      "role": "extended",
      "scope_note": "Adds the eval suites, opacity proofs and run telemetry that verify the capability's success criteria; no new negotiation behaviour."
    },
    {
      "id": "private_position_reveal_explanation",
      "role": "extended",
      "scope_note": "Adds the fixture suite and validator-coverage assertions for the reveal narration; no new behaviour."
    },
    {
      "id": "priority_sensitivity_explanation",
      "role": "extended",
      "scope_note": "Adds the flip-prediction fixture suite and grounding assertions for the counterfactual; no new behaviour."
    },
    {
      "id": "agent_message_bus",
      "role": "extended",
      "scope_note": "Adds the exhaustive cross-preset opacity proof over the bus and its stored projection; no new bus behaviour."
    }
  ],
  "tech_stack_spec": {
    "dependencies": [
    ...
    "configurations": "DATABASE_URL required for the SQL opacity proof and record assertions. OPENROUTER_API_KEY required only for the live smoke run; the golden and adversarial suites replay recorded model responses through app.dependency_overrides and must run without any provider credential. SENTRY_DSN and VITE_SENTRY_DSN remain optional and no-op when unset. Render deploy config lives in render.yaml with secrets declared sync:false and entered in the dashboard; the api service is a free Web Service that spins down after ~15 minutes idle."
  },
  "instructions": [
    "Create `backend/tests/collab/golden/` holding recorded model responses for every preset scenario across a grid of priority weightings, following the existing golden-fixture precedent in `backend/tests/golden/planning_cases.json` and `backend/tests/orchestrated/golden/`. Replay them through app.dependency_overrides so the suite is deterministic and runs with no provider credential and no quota spend.",
    "Write the golden-run assertions over every fixture: the six stages occur in the specified order; stage 1 and stage 4 consume zero model calls; negotiation_stage_call_count is exactly 6 and total_model_calls_used is exactly 8; exactly three agents participate; every bid, counter-offer and award conforms to its narrow schema; and concessions are recorded rather than empty.",
    "Write the isolation suite as an exhaustive proof rather than a sample: for every preset scenario, assert no PeerMessage has a seller as both sender and recipient; assert no artifact visible before the reveal contains any string from the rival's sealed constraint corpus; and assert each seller agent's assembled prompt contains only the RFQ, its own constraints, and buyer messages addressed to it. Iterate the full fixture set — a spot check does not prove an invariant.",
    "Add the SQL-level opacity proof as a test that runs the single predicate over the stored projection: count rows in peer_messages where both sender and recipient are sellers, asserting zero for every persisted run. This is the app's headline claim made provable from the store rather than from application state.",
    "Write the adversarial probe suite: swap in seller system-prompt variants carrying injected instructions to ask about, infer, or request the rival's position, and assert that the seller receives no rival information — because its context never contained any — and that the run still completes normally. This proves opacity is bounded by what the agent is given rather than by instruction, which is the distinction the example exists to teach.",
    "Write the sensitivity suite: for each preset scenario, assert at least one pair of priority weightings produces a different winner, and that the award rationale names the top-weighted priorities. This proves the buyer's judgment is genuinely weighting-driven rather than scenario-locked.",
    "Write the differentiation assertion: across the golden fixtures, the two opening bids differ on at least two of the four axes, so the award requires a real trade-off rather than a dominance check.",
    "Write the degradation suite: a seller timing out in a concurrent stage preserves the other track and continues in labelled degraded mode; a buyer stage failing after retries halts the run, returns the partial record with its banner, and refunds the reserved-but-unspent allowance units; a cap-exhausted request is refused before stage 1 with no partial run persisted.",
    "Add a fixture suite for the two explanation calls covering the adversarial cases their specs name — a seller that never moves, a seller that concedes below apparent value, an axis with identical bids, and a timed-out seller with a partial trajectory — asserting the deterministic validators catch stance mismatches, invented numerals, out-of-enum constraint references and rival mentions, and that the template fallback renders in every failure case.",
    "Wire the structlog telemetry the stack spec specifies for this slice: negotiation-stage call count with an alert when it differs from six, total calls used, per-stage latencies, per-seller degradation flags, explanation-call validator pass and fallback rates, and the seller-to-seller message count that must stay zero. Confirm Sentry captures errors from the PydanticAI collab lane through the existing sentry-sdk integration, which no-ops when SENTRY_DSN is unset.",
    "Run the full quality gate over the new surface and keep it clean: `uv run ruff check .`, `uv run mypy backend/app/collab backend/app/services/message_bus.py` under strict mode, `cd frontend && npm run lint`, and `cd frontend && npx tsc -b`. Do not add any new collab path to pyproject.toml's extend-exclude list and do not remove existing entries — the pre-existing legacy findings are out of scope for this revision.",
    "Verify the deploy path against render.yaml without changing the existing build command: confirm the collab slice adds no new build step, no new system binary and no new model download, and that the api service still starts within the free Web Service's constraints. Confirm no new environment variable is required and that existing secrets remain sync:false dashboard-entered.",
    "Test the cold-start path explicitly, since Render's free-tier spin-down makes it routine rather than exceptional: after an idle period, confirm the /collab overview and identity cards render before any model work begins and that the first negotiation run's SSE stream survives the cold start with sse-starlette's keep-alive pings holding the connection through Render's proxy.",
    "Run one live end-to-end smoke run against the real provider chain with a real allowance, confirming the run completes all eight calls, persists a complete NegotiationRecord, and that the model resolves through model_registry's ordered fallback chain rather than any hardcoded slug — per the code review's warning that free provider slugs rot as providers retire them.",
    "Update the README's example-app listing to include the multi-agent collaboration example, stating its 8-call budget as 6 negotiation plus 2 explanation calls and its reliance on the framework-standard shared hourly and daily allowance rather than a per-app session limit.",
    "After this phase is complete and all verification passes, create the set-completion marker so Spec4 can detect this phase set is implemented: `touch .spec4/v6/IMPLEMENTED`"
  ],
  "risk_assessment": {
    "potential_bottlenecks": "The main risk is that the eval suite becomes a suite that passes rather than a suite that proves — sampling one scenario instead of iterating every preset, or asserting the opacity invariant against in-memory application state rather than the persisted store, either of which would let a real leak through while showing green. Second, golden fixtures recorded from free models will drift as providers retire slugs, and a suite that calls live models will become flaky and will burn quota on every CI run. Third, there is a real temptation, when an assertion fails late in the project, to relax it — particularly the exactly-six-negotiation-calls assertion or a structured-output conformance check — which would erase precisely the behaviour the example exists to demonstrate. Fourth, Render's free-tier cold start can make the first post-idle request look like a hung stream.",
    "mitigation_strategy": "Write the isolation and sensitivity suites as loops over the full preset catalog rather than parameterised single cases, and make the headline opacity assertion a SQL predicate over peer_messages so it tests the stored projection rather than application memory. Replay recorded responses through app.dependency_overrides for every suite except the single live smoke run, so CI is deterministic, credential-free and quota-free while one real run still exercises the provider chain. Treat a failing call-count or conformance assertion as a defect in the implementation, never as an over-strict test — the code review's note about the single-call slice is the governing precedent: non-conformance is an expected, displayed outcome, not a bug to suppress. For cold start, assert on the first-request path after idle and on the keep-alive behaviour rather than only on process start, following the existing warm-up change-risk guidance."
  },
  "verification": "Run `uv run pytest` with no provider credential set — the entire golden, isolation, adversarial, sensitivity, differentiation, degradation and explanation-fixture suite passes on replayed responses. Confirm the isolation suite iterates every preset scenario and that the SQL proof `SELECT count(*) FROM peer_messages WHERE sender LIKE 'seller%' AND recipient LIKE 'seller%'` returns 0 across every persisted run. Confirm the golden assertions show negotiation_stage_call_count exactly 6 and total_model_calls_used exactly 8, verifying nfr_every_run_states_its_cost_in_model_calls_up_front_and_never_exceeds_its_declared_budget_ and nfr_application_wide_hourly_and_daily_usage_limits_protect_the_free_allowance_and_are_enforced_across_all_examples_together_. Confirm the adversarial suite shows a rival-probing seller receives no rival information and the run still completes, and the sensitivity suite shows a winner flip for every scenario. Run `uv run ruff check .`, `uv run mypy backend/app/collab backend/app/services/message_bus.py`, `cd frontend && npm run lint`, `cd frontend && npx tsc -b` and `cd frontend && npm run test` — all clean. Deploy to Render and, after an idle spin-down, load /collab: confirm the overview and identity cards render before any model work begins and that a live run streams all eight stages through the proxy without connection loss, verifying nfr_multi_step_runs_reveal_results_progressively_as_each_step_completes_rather_than_only_at_the_end__so_waiting_is_informative_ and nfr_intermediate_steps___retrieved_passages__plans__delegation_decisions__per_agent_messages___are_visible_to_the_visitor_rather_than_hidden__since_making_agent_behaviour_observable_is_the_point_of_the_project_. Confirm the example is reachable in one selection from the landing catalogue and the persistent nav on the deployed site, verifying nfr_every_example_is_reachable_within_one_selection_from_the_entry_view_and_from_a_persistent_navigation_listing_on_every_page_ and nfr_new_example_apps_can_be_added_and_appear_in_the_entry_view_and_navigation_without_altering_existing_examples_. Confirm structlog emits the negotiation-stage count, total calls, per-stage latencies, degradation flags and the zero seller-to-seller count for each run.",
  "references": [
    {
...
# Phase 6 of 6: Eval Suite, Opacity Proof, Telemetry and Deploy Readiness

Prove the claims the example makes. Land the golden-run and adversarial-probe suites, assert the negotiation-stage count is exactly six and the total exactly eight, prove seller-to-seller traffic is zero by SQL across every preset, demonstrate that the same scenario flips winner under different weightings, wire the structlog telemetry the stack specifies, and confirm the slice deploys cleanly on Render's free tier including the cold-start path.
...
*Scope for this phase: No new user-facing surface — this phase verifies, instruments and hardens the feature delivered in Phases 1-5.*
...
*Scope for this phase: Adds the eval suites, opacity proofs and run telemetry that verify the capability's success criteria; no new negotiation behaviour.*
...
### private_position_reveal_explanation — AI capability — extended in this phase

*Scope for this phase: Adds the fixture suite and validator-coverage assertions for the reveal narration; no new behaviour.*
...
### priority_sensitivity_explanation — AI capability — extended in this phase

*Scope for this phase: Adds the flip-prediction fixture suite and grounding assertions for the counterfactual; no new behaviour.*
...
*Scope for this phase: Adds the exhaustive cross-preset opacity proof over the bus and its stored projection; no new bus behaviour.*
...
1. Create `backend/tests/collab/golden/` holding recorded model responses for every preset scenario across a grid of priority weightings, following the existing golden-fixture precedent in `backend/tests/golden/planning_cases.json` and `backend/tests/orchestrated/golden/`. Replay them through app.dependency_overrides so the suite is deterministic and runs with no provider credential and no quota spend.
2. Write the golden-run assertions over every fixture: the six stages occur in the specified order; stage 1 and stage 4 consume zero model calls; negotiation_stage_call_count is exactly 6 and total_model_calls_used is exactly 8; exactly three agents participate; every bid, counter-offer and award conforms to its narrow schema; and concessions are recorded rather than empty.
3. Write the isolation suite as an exhaustive proof rather than a sample: for every preset scenario, assert no PeerMessage has a seller as both sender and recipient; assert no artifact visible before the reveal contains any string from the rival's sealed constraint corpus; and assert each seller agent's assembled prompt contains only the RFQ, its own constraints, and buyer messages addressed to it. Iterate the full fixture set — a spot check does not prove an invariant.
4. Add the SQL-level opacity proof as a test that runs the single predicate over the stored projection: count rows in peer_messages where both sender and recipient are sellers, asserting zero for every persisted run. This is the app's headline claim made provable from the store rather than from application state.
5. Write the adversarial probe suite: swap in seller system-prompt variants carrying injected instructions to ask about, infer, or request the rival's position, and assert that the seller receives no rival information — because its context never contained any — and that the run still completes normally. This proves opacity is bounded by what the agent is given rather than by instruction, which is the distinction the example exists to teach.
6. Write the sensitivity suite: for each preset scenario, assert at least one pair of priority weightings produces a different winner, and that the award rationale names the top-weighted priorities. This proves the buyer's judgment is genuinely weighting-driven rather than scenario-locked.
7. Write the differentiation assertion: across the golden fixtures, the two opening bids differ on at least two of the four axes, so the award requires a real trade-off rather than a dominance check.
8. Write the degradation suite: a seller timing out in a concurrent stage preserves the other track and continues in labelled degraded mode; a buyer stage failing after retries halts the run, returns the partial record with its banner, and refunds the reserved-but-unspent allowance units; a cap-exhausted request is refused before stage 1 with no partial run persisted.
9. Add a fixture suite for the two explanation calls covering the adversarial cases their specs name — a seller that never moves, a seller that concedes below apparent value, an axis with identical bids, and a timed-out seller with a partial trajectory — asserting the deterministic validators catch stance mismatches, invented numerals, out-of-enum constraint references and rival mentions, and that the template fallback renders in every failure case.
10. Wire the structlog telemetry the stack spec specifies for this slice: negotiation-stage call count with an alert when it differs from six, total calls used, per-stage latencies, per-seller degradation flags, explanation-call validator pass and fallback rates, and the seller-to-seller message count that must stay zero. Confirm Sentry captures errors from the PydanticAI collab lane through the existing sentry-sdk integration, which no-ops when SENTRY_DSN is unset.
11. Run the full quality gate over the new surface and keep it clean: `uv run ruff check .`, `uv run mypy backend/app/collab backend/app/services/message_bus.py` under strict mode, `cd frontend && npm run lint`, and `cd frontend && npx tsc -b`. Do not add any new collab path to pyproject.toml's extend-exclude list and do not remove existing entries — the pre-existing legacy findings are out of scope for this revision.
12. Verify the deploy path against render.yaml without changing the existing build command: confirm the collab slice adds no new build step, no new system binary and no new model download, and that the api service still starts within the free Web Service's constraints. Confirm no new environment variable is required and that existing secrets remain sync:false dashboard-entered.
13. Test the cold-start path explicitly, since Render's free-tier spin-down makes it routine rather than exceptional: after an idle period, confirm the /collab overview and identity cards render before any model work begins and that the first negotiation run's SSE stream survives the cold start with sse-starlette's keep-alive pings holding the connection through Render's proxy.
14. Run one live end-to-end smoke run against the real provider chain with a real allowance, confirming the run completes all eight calls, persists a complete NegotiationRecord, and that the model resolves through model_registry's ordered fallback chain rather than any hardcoded slug — per the code review's warning that free provider slugs rot as providers retire them.
15. Update the README's example-app listing to include the multi-agent collaboration example, stating its 8-call budget as 6 negotiation plus 2 explanation calls and its reliance on the framework-standard shared hourly and daily allowance rather than a per-app session limit.
16. After this phase is complete and all verification passes, create the set-completion marker so Spec4 can detect this phase set is implemented: `touch .spec4/v6/IMPLEMENTED`

## Risk Assessment

**Potential bottlenecks:**

The main risk is that the eval suite becomes a suite that passes rather than a suite that proves — sampling one scenario instead of iterating every preset, or asserting the opacity invariant against in-memory application state rather than the persisted store, either of which would let a real leak through while showing green. Second, golden fixtures recorded from free models will drift as providers retire slugs, and a suite that calls live models will become flaky and will burn quota on every CI run. Third, there is a real temptation, when an assertion fails late in the project, to relax it — particularly the exactly-six-negotiation-calls assertion or a structured-output conformance check — which would erase precisely the behaviour the example exists to demonstrate. Fourth, Render's free-tier cold start can make the first post-idle request look like a hung stream.

**Mitigation strategy:**

Write the isolation and sensitivity suites as loops over the full preset catalog rather than parameterised single cases, and make the headline opacity assertion a SQL predicate over peer_messages so it tests the stored projection rather than application memory. Replay recorded responses through app.dependency_overrides for every suite except the single live smoke run, so CI is deterministic, credential-free and quota-free while one real run still exercises the provider chain. Treat a failing call-count or conformance assertion as a defect in the implementation, never as an over-strict test — the code review's note about the single-call slice is the governing precedent: non-conformance is an expected, displayed outcome, not a bug to suppress. For cold start, assert on the first-request path after idle and on the keep-alive behaviour rather than only on process start, following the existing warm-up change-risk guidance.
...
- `nfr_all_examples_share_one_consistent_page_layout__so_a_visitor_who_learns_one_can_navigate_the_rest_without_relearning_`: All examples share one consistent page layout, so a visitor who learns one can navigate the rest without relearning. — project-wide acceptance
...
- `nfr_the_examples_remain_usable_on_a_laptop_sized_screen_with_a_typical_consumer_network_connection_`: The examples remain usable on a laptop-sized screen with a typical consumer network connection. — project-wide acceptance
