---
{
  "phase_number": 2,
  "total_phases": 6,
  "phase_title": "Sealed Fixtures, Run Persistence, the Opacity Policy, and the Zero-Call RFQ",
  "phase_summary": "Build everything the negotiation needs before a single model call exists: the pre-tuned procurement scenario catalog and sealed private constraints as mypy-checked typed literals, the negotiation_runs and peer_messages tables via a new Alembic revision, the collaboration-slice opacity policy layered over the shared message bus, the eight-call budget reservation through the existing allowance_holds service, and the deterministic QuotationRequest composer that consumes no model call. This phase makes the app's headline opacity claim true and testable before any agent can be written against a leaky bus.",
  "features": [
    {
      "id": "multi_agent_collaboration_example_app",
      "role": "extended",
      "scope_note": "Backend substrate only — fixtures, persistence, opacity policy, budget reservation and RFQ composition; the negotiation turns land in Phase 3 and all run UI in Phase 4."
    }
  ],
  "capabilities": [
    {
      ...
      "role": "extended",
      "scope_note": "Adds the collaboration slice's opacity policy layer over the generic bus built in Phase 1 — buyer/seller roles, own-constraints-only access, and the no-seller-to-seller invariant."
    },
    {
      "id": "procurement_negotiation_run",
      "role": "introduced",
      "scope_note": "Only the deterministic zero-model-call parts land here — scenario/constraint fixtures, run persistence, opacity policy, eight-call reservation and RFQ composition; the six negotiation turns and their streaming are Phase 3."
    }
  ],
  "tech_stack_spec": {
    "dependencies": [
    ...
    "configurations": "DATABASE_URL is required by this phase — the new Alembic revision runs against it and the pgvector extension is already enabled by revision 0001. CORS_ORIGIN, OPENROUTER_API_KEY and EXA_API_KEY remain required at application startup. No new environment variables are introduced. Migrations are invoked with `uv run alembic upgrade head` using backend/alembic.ini; the new revision follows the existing 11 revisions in backend/app/db/migrations/versions/."
  },
  "instructions": [
    "Extend `backend/app/collab/scenarios.py` with the procurement scenario catalog as typed Python literals — not YAML, not JSON — so mypy strict type-checks the deeply nested fixtures and no serialisation dependency is added. Author at least three scenarios. Per scenario define: a stable scenario id, the goods description, the buyer's baseline requirements and BATNA, and the four negotiable term axes (price, delivery lead time, quantity with partial-fulfilment allowance, and warranty), each axis carrying its unit and its preference direction.",
    "In the same module, author the per-scenario, per-seller sealed private constraints as typed literals: cost floor, capacity ceiling, delivery capability, and warranty liability limit, plus a reveal headline and an explanation seed for the end-of-run unsealing. Hand-tune each scenario so the two sellers' constraints force genuinely non-comparable bids — for example one seller cheap but capacity-constrained with a short warranty tolerance, the other pricier but fast with large capacity and long warranty headroom. Keep these in the same module as the scenario they belong to: the sealing is enforced by the opacity policy at access time, not by file separation, and splitting files would imply a boundary the filesystem is not providing.",
    "In the same module, author the selectable priority-weighting presets, each carrying its per-axis weights, chosen so that at least one pair of weightings flips the winner for every scenario. Weights are validated server-side as a closed numeric vector over the scenario's declared axes — there is no free-text input anywhere in this example.",
    "Create a new Alembic revision in `backend/app/db/migrations/versions/` following the existing numbering convention after 0011_moderation_log, adding two tables. `negotiation_runs`: id primary key, created_at with an index, scenario_id with an index, a negotiation_stage_call_count column and a total_model_calls_used column as queryable top-level integers (deliberately NOT buried in JSONB), a JSONB column each for the RFQ, opening bids, counter-offers, final bids, award, reveal payload and sensitivity payload, a JSONB stage-timing column, and a JSONB degradation-flags column. `peer_messages`: id primary key, a foreign key to negotiation_runs.id, a sequence column unique per run, sender and recipient columns indexed together so seller-to-seller traffic is queryable by a single SQL predicate, a stage column, and a JSONB work-item column carrying the A2A-shaped payload.",
    "Add the corresponding SQLAlchemy models to `backend/app/db/` following the existing model conventions in that package. Name the entities to match the project's domain vocabulary exactly: NegotiationRecord, PeerMessage, MessageLog, QuotationRequest, Bid, CounterOffer, Award, PrivateConstraint, Scenario, PriorityWeighting, Run. Do not invent synonyms for concepts the vocabulary already names.",
    "Create `backend/app/collab/opacity.py` implementing the collaboration slice's opacity policy as a layer over the shared `backend/app/services/message_bus.py` built in Phase 1 — the policy belongs to the slice, the generic bus stays generic. Implement: a role model distinguishing the buyer agent from the two seller agents; a `constraints_for(agent_id)` function that can only ever return that agent's own sealed constraints and has no code path returning another party's; and a `assemble_context(agent_id)` function that builds an agent's turn context strictly from `context_for(agent_id)` on the bus plus that agent's own constraints plus the public RFQ.",
    "Write `assemble_context` so a rival's material is unreachable by construction rather than by instruction: the function is never handed the rival's envelopes or constraints, so no prompt built from it can contain them even if the model reasons its way toward asking. Add no parameter, flag or override that could widen what it returns.",
    "Implement a `deliver` wrapper in opacity.py that enforces the no-seller-to-seller invariant structurally: any attempt to deliver an envelope whose sender and recipient are both sellers raises immediately and is never appended to the bus. This is an invariant violation, not a recoverable condition.",
    "Implement the outbound leak lint as a pure function in opacity.py: before an envelope is delivered or streamed, check its text content against the rival party's sealed constraint corpus using exact matching on constraint values and their common string renderings. A hit is a hard safety stop — the run aborts, the offending artifact is never emitted to the client, and a structlog error event carrying the run id fires. Place the lint in the delivery path so it cannot be bypassed by an agent.",
    "Implement the deterministic RFQ composer in `backend/app/collab/` as a pure function taking the selected scenario and the visitor's priority weighting and returning a QuotationRequest — with no model call whatsoever. Deterministic templating here is what keeps the negotiation stage count at exactly six and demonstrates that not every step in an agent system needs a model; say so in the function's docstring.",
    "Wire the eight-call budget reservation through the EXISTING `backend/app/services/allowance_holds.py` service — do not write a parallel counter. Reserve all eight units atomically before the RFQ is composed, so a run is never begun that cannot complete. The eight are 6 negotiation calls plus 2 post-award explanation calls, per the stack spec's collaboration_run_call_budget decision. On refusal, return a distinguishable 'usage cap reached' outcome carrying remaining allowance and reset time — never a generic error, and never a partially started run. Implement refund of reserved-but-unspent units on run failure.",
    "Add a scenario-and-weighting validation function that rejects any scenario id outside the catalog enum and any weighting vector that does not match the scenario's declared axes, server-side, before any reservation is taken. Both inputs are closed enumerations and numeric vectors — no visitor free text reaches this slice at all, which is why this example needs no moderation gate.",
    "Add structlog telemetry fields for this slice as the stack spec specifies: negotiation-stage call count, total calls used, per-stage latencies, per-seller degradation flags, and the seller-to-seller message count that must stay zero.",
    "Write pytest tests in `backend/tests/` covering: that `constraints_for` returns only the requesting agent's constraints for every agent across every preset scenario; that `assemble_context` for a seller contains no string from the rival's constraint corpus, across every preset; that delivering a seller-to-seller envelope raises and does not append; that the leak lint catches a planted rival constraint value in an outbound message; that the RFQ composer is deterministic (same inputs produce identical output) and consumes zero model calls; that the eight-call reservation is taken atomically before RFQ composition and refuses up front when allowance is short; and that reserved units are refunded when a run fails before spending them.",
    "Write a test asserting that every preset scenario has at least one pair of priority weightings under which the deterministic per-axis weighted scoring of hand-authored representative bids ranks a different seller first — proving the fixtures are tuned for genuine non-comparability before any model is involved.",
    "Run `uv run alembic upgrade head` and confirm both tables are created; verify `CREATE EXTENSION vector` privileges are already satisfied on the target database, since the existing revision 0001 depends on them and a restricted Postgres fails at the first revision.",
    "Keep `backend/app/collab/` and the new tests ruff-clean and mypy-strict without adding them to pyproject.toml's extend-exclude list and without removing existing entries from it."
  ],
  "risk_assessment": {
    "potential_bottlenecks": "The deepest risk is that the opacity guarantee becomes cosmetic — a policy function that filters correctly today but is bypassed later by a caller reaching around it to the raw fixture module, since the sealed constraints live in the same importable file as the scenarios. Second, the fixtures are the load-bearing part of the demo: if the two sellers' constraints are not genuinely orthogonal, every downstream phase produces bids that look comparable and the whole teaching point collapses, but this will not be visible until Phase 3 runs real models. Third, allowance-hold logic is easy to get subtly wrong — reserving after composing rather than before, forgetting to refund on the failure path, or keying the hold to the wrong UTC window so it drifts from the existing hourly gate. Fourth, the Alembic revision must chain correctly onto revision 0011 and will fail confusingly if down_revision is mis-set.",
    "mitigation_strategy": "For opacity, make the policy the only sanctioned access path and prove it with tests that iterate every preset scenario rather than checking one happy case — the test asserting no rival string appears in any seller's assembled context is the guarantee's real enforcement, and it must be exhaustive over the fixture set. For fixture tuning, add the deterministic winner-flip test in this phase using hand-authored representative bids, so a badly tuned scenario fails in CI now rather than surfacing as a vague quality problem after model calls are burned in Phase 3. For allowance holds, reuse the existing allowance_holds service rather than writing new counting logic — the code review's change-risk note is explicit that new code calling providers or counting quota outside the shared gate silently escapes accounting — and test the reserve-before-compose ordering and the refund path directly. For Alembic, set down_revision to the 0011_moderation_log revision id explicitly and verify with `uv run alembic history` before upgrading."
  },
  "verification": "Run `uv run alembic upgrade head` and confirm the new revision applies cleanly; inspect the database and confirm negotiation_runs carries negotiation_stage_call_count and total_model_calls_used as top-level queryable integer columns and that peer_messages has a composite index on sender and recipient. Run `uv run pytest` — all opacity, leak-lint, RFQ-determinism, reservation, refund and fixture-tuning tests pass. Run `uv run mypy backend/app/collab` and confirm the typed scenario, sealed-constraint and identity-card literals type-check under strict mode with no errors. Run `uv run ruff check .` and confirm clean. Confirm that no code path outside backend/app/collab/opacity.py reads another party's sealed constraints by grepping for direct imports of the constraint literals. Verify nfr_application_wide_hourly_and_daily_usage_limits_protect_the_free_allowance_and_are_enforced_across_all_examples_together_ by confirming the eight-call hold is taken through the existing shared allowance_holds service against the same UTC-hour window the other examples use, and nfr_every_run_states_its_cost_in_model_calls_up_front_and_never_exceeds_its_declared_budget_ by confirming the reservation is taken before RFQ composition and that a short allowance refuses the run before any stage begins.",
  "references": [
    {
...
# Phase 2 of 6: Sealed Fixtures, Run Persistence, the Opacity Policy, and the Zero-Call RFQ

Build everything the negotiation needs before a single model call exists: the pre-tuned procurement scenario catalog and sealed private constraints as mypy-checked typed literals, the negotiation_runs and peer_messages tables via a new Alembic revision, the collaboration-slice opacity policy layered over the shared message bus, the eight-call budget reservation through the existing allowance_holds service, and the deterministic QuotationRequest composer that consumes no model call. This phase makes the app's headline opacity claim true and testable before any agent can be written against a leaky bus.
...
*Scope for this phase: Backend substrate only — fixtures, persistence, opacity policy, budget reservation and RFQ composition; the negotiation turns land in Phase 3 and all run UI in Phase 4.*
...
### agent_message_bus — AI capability — extended in this phase

*Scope for this phase: Adds the collaboration slice's opacity policy layer over the generic bus built in Phase 1 — buyer/seller roles, own-constraints-only access, and the no-seller-to-seller invariant.*
...
### procurement_negotiation_run — AI capability — introduced in this phase

*Scope for this phase: Only the deterministic zero-model-call parts land here — scenario/constraint fixtures, run persistence, opacity policy, eight-call reservation and RFQ composition; the six negotiation turns and their streaming are Phase 3.*
...
1. Extend `backend/app/collab/scenarios.py` with the procurement scenario catalog as typed Python literals — not YAML, not JSON — so mypy strict type-checks the deeply nested fixtures and no serialisation dependency is added. Author at least three scenarios. Per scenario define: a stable scenario id, the goods description, the buyer's baseline requirements and BATNA, and the four negotiable term axes (price, delivery lead time, quantity with partial-fulfilment allowance, and warranty), each axis carrying its unit and its preference direction.
2. In the same module, author the per-scenario, per-seller sealed private constraints as typed literals: cost floor, capacity ceiling, delivery capability, and warranty liability limit, plus a reveal headline and an explanation seed for the end-of-run unsealing. Hand-tune each scenario so the two sellers' constraints force genuinely non-comparable bids — for example one seller cheap but capacity-constrained with a short warranty tolerance, the other pricier but fast with large capacity and long warranty headroom. Keep these in the same module as the scenario they belong to: the sealing is enforced by the opacity policy at access time, not by file separation, and splitting files would imply a boundary the filesystem is not providing.
3. In the same module, author the selectable priority-weighting presets, each carrying its per-axis weights, chosen so that at least one pair of weightings flips the winner for every scenario. Weights are validated server-side as a closed numeric vector over the scenario's declared axes — there is no free-text input anywhere in this example.
4. Create a new Alembic revision in `backend/app/db/migrations/versions/` following the existing numbering convention after 0011_moderation_log, adding two tables. `negotiation_runs`: id primary key, created_at with an index, scenario_id with an index, a negotiation_stage_call_count column and a total_model_calls_used column as queryable top-level integers (deliberately NOT buried in JSONB), a JSONB column each for the RFQ, opening bids, counter-offers, final bids, award, reveal payload and sensitivity payload, a JSONB stage-timing column, and a JSONB degradation-flags column. `peer_messages`: id primary key, a foreign key to negotiation_runs.id, a sequence column unique per run, sender and recipient columns indexed together so seller-to-seller traffic is queryable by a single SQL predicate, a stage column, and a JSONB work-item column carrying the A2A-shaped payload.
5. Add the corresponding SQLAlchemy models to `backend/app/db/` following the existing model conventions in that package. Name the entities to match the project's domain vocabulary exactly: NegotiationRecord, PeerMessage, MessageLog, QuotationRequest, Bid, CounterOffer, Award, PrivateConstraint, Scenario, PriorityWeighting, Run. Do not invent synonyms for concepts the vocabulary already names.
6. Create `backend/app/collab/opacity.py` implementing the collaboration slice's opacity policy as a layer over the shared `backend/app/services/message_bus.py` built in Phase 1 — the policy belongs to the slice, the generic bus stays generic. Implement: a role model distinguishing the buyer agent from the two seller agents; a `constraints_for(agent_id)` function that can only ever return that agent's own sealed constraints and has no code path returning another party's; and a `assemble_context(agent_id)` function that builds an agent's turn context strictly from `context_for(agent_id)` on the bus plus that agent's own constraints plus the public RFQ.
7. Write `assemble_context` so a rival's material is unreachable by construction rather than by instruction: the function is never handed the rival's envelopes or constraints, so no prompt built from it can contain them even if the model reasons its way toward asking. Add no parameter, flag or override that could widen what it returns.
8. Implement a `deliver` wrapper in opacity.py that enforces the no-seller-to-seller invariant structurally: any attempt to deliver an envelope whose sender and recipient are both sellers raises immediately and is never appended to the bus. This is an invariant violation, not a recoverable condition.
9. Implement the outbound leak lint as a pure function in opacity.py: before an envelope is delivered or streamed, check its text content against the rival party's sealed constraint corpus using exact matching on constraint values and their common string renderings. A hit is a hard safety stop — the run aborts, the offending artifact is never emitted to the client, and a structlog error event carrying the run id fires. Place the lint in the delivery path so it cannot be bypassed by an agent.
10. Implement the deterministic RFQ composer in `backend/app/collab/` as a pure function taking the selected scenario and the visitor's priority weighting and returning a QuotationRequest — with no model call whatsoever. Deterministic templating here is what keeps the negotiation stage count at exactly six and demonstrates that not every step in an agent system needs a model; say so in the function's docstring.
11. Wire the eight-call budget reservation through the EXISTING `backend/app/services/allowance_holds.py` service — do not write a parallel counter. Reserve all eight units atomically before the RFQ is composed, so a run is never begun that cannot complete. The eight are 6 negotiation calls plus 2 post-award explanation calls, per the stack spec's collaboration_run_call_budget decision. On refusal, return a distinguishable 'usage cap reached' outcome carrying remaining allowance and reset time — never a generic error, and never a partially started run. Implement refund of reserved-but-unspent units on run failure.
12. Add a scenario-and-weighting validation function that rejects any scenario id outside the catalog enum and any weighting vector that does not match the scenario's declared axes, server-side, before any reservation is taken. Both inputs are closed enumerations and numeric vectors — no visitor free text reaches this slice at all, which is why this example needs no moderation gate.
13. Add structlog telemetry fields for this slice as the stack spec specifies: negotiation-stage call count, total calls used, per-stage latencies, per-seller degradation flags, and the seller-to-seller message count that must stay zero.
14. Write pytest tests in `backend/tests/` covering: that `constraints_for` returns only the requesting agent's constraints for every agent across every preset scenario; that `assemble_context` for a seller contains no string from the rival's constraint corpus, across every preset; that delivering a seller-to-seller envelope raises and does not append; that the leak lint catches a planted rival constraint value in an outbound message; that the RFQ composer is deterministic (same inputs produce identical output) and consumes zero model calls; that the eight-call reservation is taken atomically before RFQ composition and refuses up front when allowance is short; and that reserved units are refunded when a run fails before spending them.
15. Write a test asserting that every preset scenario has at least one pair of priority weightings under which the deterministic per-axis weighted scoring of hand-authored representative bids ranks a different seller first — proving the fixtures are tuned for genuine non-comparability before any model is involved.
16. Run `uv run alembic upgrade head` and confirm both tables are created; verify `CREATE EXTENSION vector` privileges are already satisfied on the target database, since the existing revision 0001 depends on them and a restricted Postgres fails at the first revision.
17. Keep `backend/app/collab/` and the new tests ruff-clean and mypy-strict without adding them to pyproject.toml's extend-exclude list and without removing existing entries from it.

## Risk Assessment

**Potential bottlenecks:**

The deepest risk is that the opacity guarantee becomes cosmetic — a policy function that filters correctly today but is bypassed later by a caller reaching around it to the raw fixture module, since the sealed constraints live in the same importable file as the scenarios. Second, the fixtures are the load-bearing part of the demo: if the two sellers' constraints are not genuinely orthogonal, every downstream phase produces bids that look comparable and the whole teaching point collapses, but this will not be visible until Phase 3 runs real models. Third, allowance-hold logic is easy to get subtly wrong — reserving after composing rather than before, forgetting to refund on the failure path, or keying the hold to the wrong UTC window so it drifts from the existing hourly gate. Fourth, the Alembic revision must chain correctly onto revision 0011 and will fail confusingly if down_revision is mis-set.

**Mitigation strategy:**

For opacity, make the policy the only sanctioned access path and prove it with tests that iterate every preset scenario rather than checking one happy case — the test asserting no rival string appears in any seller's assembled context is the guarantee's real enforcement, and it must be exhaustive over the fixture set. For fixture tuning, add the deterministic winner-flip test in this phase using hand-authored representative bids, so a badly tuned scenario fails in CI now rather than surfacing as a vague quality problem after model calls are burned in Phase 3. For allowance holds, reuse the existing allowance_holds service rather than writing new counting logic — the code review's change-risk note is explicit that new code calling providers or counting quota outside the shared gate silently escapes accounting — and test the reserve-before-compose ordering and the refund path directly. For Alembic, set down_revision to the 0011_moderation_log revision id explicitly and verify with `uv run alembic history` before upgrading.
