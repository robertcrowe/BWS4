---
{
  "phase_number": 6,
  "total_phases": 7,
  "phase_title": "Orchestrated Subagents Screen — Six Surfaces, SSE Consumption, and the Client Run Allowance",
  "phase_summary": "Build the complete screen-orchestrated UI against the finalized mock: the pattern overview, the four-specialist roster panel, the question form with preset chips and moderation refusal messaging, the delegation review with its explicit dispatch control, two live parallel specialist columns, and the markdown merged answer with its disagreement note — plus the hour-stamped localStorage run counter, rehydrated prior runs, and the two distinct exhaustion messages.",
  "features": [
    {
      "id": "orchestrated_subagents_example_app",
      "role": "extended",
      "scope_note": "All six UI surfaces of screen-orchestrated land here along with the client-side run allowance; only catalogue registration, the golden-set eval and run telemetry remain, in Phase 7."
    }
  ],
  "capabilities": [
    {
      ...
      "role": "extended",
      "scope_note": "Adds the visitor-facing realization of the capability — the delegation review, the two concurrently-streaming columns, and the merged answer — over the backend built in Phases 3-5."
    },
    {
      "id": "specialist_disagreement_note",
      "role": "extended",
      "scope_note": "Adds the note's distinct UI panel with per-item attribution and the degraded-mode and fallback copy."
    },
    {
      "id": "question_moderation",
      "role": "extended",
      "scope_note": "Adds the visitor-facing refusal message rendered as plain text, with the input left enabled and the run counter untouched."
    }
  ],
  "tech_stack_spec": {
    "dependencies": [
    ...
    "configurations": "The SPA calls the API over HTTPS only, at the origin the API's CORS_ORIGIN permits. VITE_SENTRY_DSN remains optional and Sentry stays uninitialized when unset. The orchestrated route is a React.lazy chunk registered in frontend/src/routes.tsx so the gallery grows without bloating the initial load. The localStorage key for the orchestrated run allowance is a named constant, distinct from the planning app's existing run_allowance key and from theme_preference."
  },
  "instructions": [
    "Reference .spec4/v5/design/mock.html throughout this phase as the authoritative visual guide for screen-orchestrated. Match its layout, spacing, column arrangement, and the specialist column colours to the roster config's colour field.",
    "Build all six surfaces inside frontend/src/apps/orchestrated/, replacing the Phase 1 placeholder route: orchestrated_overview, specialist_roster_panel, orchestrated_question_form, orchestrated_delegation_review, orchestrated_parallel_execution, and orchestrated_merged_answer. Render them within the existing shared layout shell and persistent nav from frontend/src/components/ — do not create a parallel layout for this app.",
    "Build orchestrated_overview as a non-AI educational panel explaining the orchestrated subagents pattern: that a coordinator delegates to independent workers, and that the two subagents can run at the same time precisely because neither depends on the other's output. State plainly that the fixed three-call budget and the three-run session limit are quota-conservation choices for this demonstration, while the pattern itself supports any number of agents.",
    "Build specialist_roster_panel as a non-AI panel listing all four specialists — Technical, Financial, Historical, Practical — each with its display name, one-line scope and column colour, fetched from the Phase 1 roster endpoint. This panel is what lets the visitor see the closed set the coordinator must choose two from.",
    "Build orchestrated_question_form with the curated preset questions as prominent one-click chips plus a free-form text input. Enforce the input length cap from the specification client-side with a visible limit and prevent submission of empty input. Show a 'checking your question' state from the moment of submit so the moderation wait is visible.",
    "Render a moderation refusal as plain text only — never as HTML and never through the markdown renderer. This closes the reflected-injection path into the page. On refusal, leave the input enabled, keep the visitor's text intact, leave prior results on screen, and do not decrement the run counter.",
    "Build orchestrated_delegation_review to display the coordinator's chosen pair, the pairing rationale, and each specialist's full brief, with an explicit dispatch button. Nothing may be dispatched until the visitor presses it — the confirmation gate is what makes the fan-out visible and deliberate. Surface the fit-quality caveat honestly when the coordinator reports a weak fit, keeping the preset chips visible as a suggested alternative.",
    "Build orchestrated_parallel_execution as two side-by-side columns, each headed by that specialist's own brief so the visitor can see the two agents received different instructions. Bind each column to its own independent SSE event stream state so both show as in-progress simultaneously; never gate one column's render on the other's completion. On smaller screens the columns stack vertically while remaining clearly two distinct workers.",
    "Render a per-column status independently for the running, complete, failed and timed-out states. When one column fails, keep the other column's answer fully visible and show the failed column's state clearly rather than removing it.",
    "Build orchestrated_merged_answer to render the merged text and the specialist answers through react-markdown, never via dangerouslySetInnerHTML — model output derived from visitor free-form input must not be able to inject HTML or script on this unauthenticated public surface. Use the shared markdown renderer wrapper in frontend/src/components/ with its Tailwind typography styles, creating it if it does not yet exist. Do not add remark-gfm; it is marked optional in the stack and is out of scope.",
    "Render the disagreement note as its own distinct panel beside the merged answer, showing agreements, complements and contradictions with per-item specialist attribution. In degraded mode render the fixed single-specialist copy; on note failure render the fallback copy while still showing the merged answer.",
    "Show a standing 'AI-generated demonstration output, not advice' disclaimer on the specialist answers and the merged answer, and tell visitors in the form area not to enter personal or confidential information.",
    "Implement the run allowance as a custom hook in frontend/src/apps/orchestrated/ following the project's custom-hook convention, backed by the orchestrated_run_allowance localStorage record. Stamp the record with the current UTC hour so the three-run counter resets on the same hourly clock as the server-side showcase-wide gate, and reset the counter when the stored hour stamp no longer matches the current UTC hour.",
    "Persist the visitor's completed run records — question, delegation decision, per-specialist briefs, specialist answers and merged answer — into the same localStorage record, and rehydrate them on page load so the runs-remaining count and previously produced results survive navigating away and back. There is no server-side visitor identity; this store is what makes that work.",
    "Display the runs-remaining count visibly on the page at all times and decrement it by exactly one per completed run. At zero, disable the question input and the dispatch control with an explanatory message while leaving all prior results rendered.",
    "Hold the two exhaustion messages as two separate named constants with distinct wording: one for this app's own three-run session limit, sourced from the local counter, and one for the showcase-wide hourly allowance being exhausted, sourced from the server's distinct refusal outcome. They must never collapse into one string.",
    "Note in the UI copy that this store is a per-device convenience and that the real protection is the server-side allowance gate, consistent with how the existing planning app presents its advisory counter.",
    "Consume the run stream with @microsoft/fetch-event-source rather than the browser's native EventSource, which is GET-only and cannot carry this run's POST payload. Abort the stream on component unmount so an abandoned run stops spending model quota.",
    "Write Vitest + React Testing Library tests in frontend/tests/ or alongside the components per the project's convention: the roster panel renders all four specialists; a moderation refusal renders as plain text, leaves the input enabled and does not decrement the counter; no dispatch request is issued until the dispatch control is pressed; both columns render an in-progress state simultaneously from interleaved mock SSE events; one column failing leaves the other's answer visible; the merged answer renders markdown as elements with no dangerouslySetInnerHTML anywhere in the tree; the two exhaustion messages are distinct strings and each appears in its own scenario; at zero runs the input and dispatch control are disabled while prior results remain; and prior runs rehydrate from a seeded hour-stamped localStorage record while a stale hour stamp resets the counter.",
    "Every new source file opens with the header comment `Built with Spec4 AI - https://spec4.ai`; JSDoc on every exported TypeScript function."
  ],
  "risk_assessment": {
    "potential_bottlenecks": "The most likely UI failure is rendering the two columns from a single combined state object, which makes them update together and destroys the visible parallelism that is the whole point of the screen. A second is rendering model-derived markdown through dangerouslySetInnerHTML, which on an unauthenticated public surface accepting free-form input is a genuine XSS path. A third is collapsing the two exhaustion messages into one generic 'limit reached' string, which the specification explicitly forbids. localStorage hour-stamping is easy to get wrong by using local time instead of UTC, causing the client counter to drift out of step with the server's UTC-hour gate. An AI coder may also reach for the native EventSource, which cannot carry the POST payload this run requires. Finally, an unaborted stream on unmount keeps burning model quota after the visitor navigates away.",
    "mitigation_strategy": "Model the two columns as two independent state entries keyed by specialist id, updated by their own events, and test the concurrency with interleaved mock SSE events asserting both show in-progress at once. Route all model-derived text through the shared react-markdown wrapper and add a test asserting dangerouslySetInnerHTML appears nowhere in the rendered tree. Keep the two exhaustion strings as separate exported constants and assert both independently. Derive the hour stamp from UTC explicitly and test the reset with a seeded stale record. Use @microsoft/fetch-event-source, which the stack approves precisely because native EventSource is GET-only. Wire the stream's abort controller into the component's unmount cleanup and test that unmount aborts."
  },
  "verification": "Run `cd frontend && npm test` — all new orchestrated component tests pass, including the simultaneous two-column in-progress assertion, the plain-text moderation refusal, the no-dispatch-before-confirmation assertion, the distinct exhaustion strings, the zero-runs disabled state with prior results retained, and localStorage rehydration with stale-hour reset. Run `cd frontend && npm run build` and confirm tsc -b succeeds and a separate lazy chunk is emitted for the orchestrated route. Run `cd frontend && npm run lint` with zero findings. With the API running, open /orchestrated in a browser and complete a full run: pick a preset, see the delegation decision with both briefs, press dispatch, watch both columns progress at the same time, and see the merged answer with its disagreement note. Confirm the runs-remaining count decrements by one, then navigate away and back and confirm the count and prior results are still shown. Narrow the browser to a small-screen width and confirm the columns stack while remaining legible. Verify nfr_all_example_apps_share_one_consistent_layout_and_one_consistent_navigation__so_moving_between_them_requires_no_relearning and nfr_usable_in_a_browser_on_ordinary_consumer_hardware__including_on_smaller_screens by confirming the screen renders inside the shared layout shell and reflows at small widths; nfr_every_example_app_is_understandable_to_a_developer_with_no_prior_exposure_to_the_pattern_within_a_couple_of_minutes_of_opening_it via the overview panel and per-column briefs; nfr_usage_limits_are_always_explained_in_plain_language__distinguishing_a_single_app_s_own_demonstration_limit_from_the_showcase_wide_daily_allowance via the two distinct exhaustion strings; nfr_visitors_need_no_sign_up_or_credentials_of_their_own_to_explore_any_example by confirming a complete run requires no credentials and sets no identity cookie; and nfr_when_a_model_or_an_external_lookup_is_unavailable__the_affected_example_degrades_visibly_and_gracefully__keeping_already_produced_results_on_screen via the single-column-failure test.",
  "references": [
    {
      "standard": "React",
      "url": "https://react.dev/"
    ...
    {
      "standard": "MDN — Window.localStorage",
...
# Phase 6 of 7: Orchestrated Subagents Screen — Six Surfaces, SSE Consumption, and the Client Run Allowance

Build the complete screen-orchestrated UI against the finalized mock: the pattern overview, the four-specialist roster panel, the question form with preset chips and moderation refusal messaging, the delegation review with its explicit dispatch control, two live parallel specialist columns, and the markdown merged answer with its disagreement note — plus the hour-stamped localStorage run counter, rehydrated prior runs, and the two distinct exhaustion messages.
...
*Scope for this phase: All six UI surfaces of screen-orchestrated land here along with the client-side run allowance; only catalogue registration, the golden-set eval and run telemetry remain, in Phase 7.*
...
*Scope for this phase: Adds the visitor-facing realization of the capability — the delegation review, the two concurrently-streaming columns, and the merged answer — over the backend built in Phases 3-5.*
...
*Scope for this phase: Adds the note's distinct UI panel with per-item attribution and the degraded-mode and fallback copy.*
...
*Scope for this phase: Adds the visitor-facing refusal message rendered as plain text, with the input left enabled and the run counter untouched.*
...
1. Reference .spec4/v5/design/mock.html throughout this phase as the authoritative visual guide for screen-orchestrated. Match its layout, spacing, column arrangement, and the specialist column colours to the roster config's colour field.
2. Build all six surfaces inside frontend/src/apps/orchestrated/, replacing the Phase 1 placeholder route: orchestrated_overview, specialist_roster_panel, orchestrated_question_form, orchestrated_delegation_review, orchestrated_parallel_execution, and orchestrated_merged_answer. Render them within the existing shared layout shell and persistent nav from frontend/src/components/ — do not create a parallel layout for this app.
3. Build orchestrated_overview as a non-AI educational panel explaining the orchestrated subagents pattern: that a coordinator delegates to independent workers, and that the two subagents can run at the same time precisely because neither depends on the other's output. State plainly that the fixed three-call budget and the three-run session limit are quota-conservation choices for this demonstration, while the pattern itself supports any number of agents.
4. Build specialist_roster_panel as a non-AI panel listing all four specialists — Technical, Financial, Historical, Practical — each with its display name, one-line scope and column colour, fetched from the Phase 1 roster endpoint. This panel is what lets the visitor see the closed set the coordinator must choose two from.
5. Build orchestrated_question_form with the curated preset questions as prominent one-click chips plus a free-form text input. Enforce the input length cap from the specification client-side with a visible limit and prevent submission of empty input. Show a 'checking your question' state from the moment of submit so the moderation wait is visible.
6. Render a moderation refusal as plain text only — never as HTML and never through the markdown renderer. This closes the reflected-injection path into the page. On refusal, leave the input enabled, keep the visitor's text intact, leave prior results on screen, and do not decrement the run counter.
7. Build orchestrated_delegation_review to display the coordinator's chosen pair, the pairing rationale, and each specialist's full brief, with an explicit dispatch button. Nothing may be dispatched until the visitor presses it — the confirmation gate is what makes the fan-out visible and deliberate. Surface the fit-quality caveat honestly when the coordinator reports a weak fit, keeping the preset chips visible as a suggested alternative.
8. Build orchestrated_parallel_execution as two side-by-side columns, each headed by that specialist's own brief so the visitor can see the two agents received different instructions. Bind each column to its own independent SSE event stream state so both show as in-progress simultaneously; never gate one column's render on the other's completion. On smaller screens the columns stack vertically while remaining clearly two distinct workers.
9. Render a per-column status independently for the running, complete, failed and timed-out states. When one column fails, keep the other column's answer fully visible and show the failed column's state clearly rather than removing it.
10. Build orchestrated_merged_answer to render the merged text and the specialist answers through react-markdown, never via dangerouslySetInnerHTML — model output derived from visitor free-form input must not be able to inject HTML or script on this unauthenticated public surface. Use the shared markdown renderer wrapper in frontend/src/components/ with its Tailwind typography styles, creating it if it does not yet exist. Do not add remark-gfm; it is marked optional in the stack and is out of scope.
11. Render the disagreement note as its own distinct panel beside the merged answer, showing agreements, complements and contradictions with per-item specialist attribution. In degraded mode render the fixed single-specialist copy; on note failure render the fallback copy while still showing the merged answer.
12. Show a standing 'AI-generated demonstration output, not advice' disclaimer on the specialist answers and the merged answer, and tell visitors in the form area not to enter personal or confidential information.
13. Implement the run allowance as a custom hook in frontend/src/apps/orchestrated/ following the project's custom-hook convention, backed by the orchestrated_run_allowance localStorage record. Stamp the record with the current UTC hour so the three-run counter resets on the same hourly clock as the server-side showcase-wide gate, and reset the counter when the stored hour stamp no longer matches the current UTC hour.
14. Persist the visitor's completed run records — question, delegation decision, per-specialist briefs, specialist answers and merged answer — into the same localStorage record, and rehydrate them on page load so the runs-remaining count and previously produced results survive navigating away and back. There is no server-side visitor identity; this store is what makes that work.
15. Display the runs-remaining count visibly on the page at all times and decrement it by exactly one per completed run. At zero, disable the question input and the dispatch control with an explanatory message while leaving all prior results rendered.
16. Hold the two exhaustion messages as two separate named constants with distinct wording: one for this app's own three-run session limit, sourced from the local counter, and one for the showcase-wide hourly allowance being exhausted, sourced from the server's distinct refusal outcome. They must never collapse into one string.
17. Note in the UI copy that this store is a per-device convenience and that the real protection is the server-side allowance gate, consistent with how the existing planning app presents its advisory counter.
18. Consume the run stream with @microsoft/fetch-event-source rather than the browser's native EventSource, which is GET-only and cannot carry this run's POST payload. Abort the stream on component unmount so an abandoned run stops spending model quota.
19. Write Vitest + React Testing Library tests in frontend/tests/ or alongside the components per the project's convention: the roster panel renders all four specialists; a moderation refusal renders as plain text, leaves the input enabled and does not decrement the counter; no dispatch request is issued until the dispatch control is pressed; both columns render an in-progress state simultaneously from interleaved mock SSE events; one column failing leaves the other's answer visible; the merged answer renders markdown as elements with no dangerouslySetInnerHTML anywhere in the tree; the two exhaustion messages are distinct strings and each appears in its own scenario; at zero runs the input and dispatch control are disabled while prior results remain; and prior runs rehydrate from a seeded hour-stamped localStorage record while a stale hour stamp resets the counter.
20. Every new source file opens with the header comment `Built with Spec4 AI - https://spec4.ai`; JSDoc on every exported TypeScript function.

## Risk Assessment

**Potential bottlenecks:**

The most likely UI failure is rendering the two columns from a single combined state object, which makes them update together and destroys the visible parallelism that is the whole point of the screen. A second is rendering model-derived markdown through dangerouslySetInnerHTML, which on an unauthenticated public surface accepting free-form input is a genuine XSS path. A third is collapsing the two exhaustion messages into one generic 'limit reached' string, which the specification explicitly forbids. localStorage hour-stamping is easy to get wrong by using local time instead of UTC, causing the client counter to drift out of step with the server's UTC-hour gate. An AI coder may also reach for the native EventSource, which cannot carry the POST payload this run requires. Finally, an unaborted stream on unmount keeps burning model quota after the visitor navigates away.
...
Run `cd frontend && npm test` — all new orchestrated component tests pass, including the simultaneous two-column in-progress assertion, the plain-text moderation refusal, the no-dispatch-before-confirmation assertion, the distinct exhaustion strings, the zero-runs disabled state with prior results retained, and localStorage rehydration with stale-hour reset. Run `cd frontend && npm run build` and confirm tsc -b succeeds and a separate lazy chunk is emitted for the orchestrated route. Run `cd frontend && npm run lint` with zero findings. With the API running, open /orchestrated in a browser and complete a full run: pick a preset, see the delegation decision with both briefs, press dispatch, watch both columns progress at the same time, and see the merged answer with its disagreement note. Confirm the runs-remaining count decrements by one, then navigate away and back and confirm the count and prior results are still shown. Narrow the browser to a small-screen width and confirm the columns stack while remaining legible. Verify nfr_all_example_apps_share_one_consistent_layout_and_one_consistent_navigation__so_moving_between_them_requires_no_relearning and nfr_usable_in_a_browser_on_ordinary_consumer_hardware__including_on_smaller_screens by confirming the screen renders inside the shared layout shell and reflows at small widths; nfr_every_example_app_is_understandable_to_a_developer_with_no_prior_exposure_to_the_pattern_within_a_couple_of_minutes_of_opening_it via the overview panel and per-column briefs; nfr_usage_limits_are_always_explained_in_plain_language__distinguishing_a_single_app_s_own_demonstration_limit_from_the_showcase_wide_daily_allowance via the two distinct exhaustion strings; nfr_visitors_need_no_sign_up_or_credentials_of_their_own_to_explore_any_example by confirming a complete run requires no credentials and sets no identity cookie; and nfr_when_a_model_or_an_external_lookup_is_unavailable__the_affected_example_degrades_visibly_and_gracefully__keeping_already_produced_results_on_screen via the single-column-failure test.
