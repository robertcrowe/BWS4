...
## Overview
<a href="https://spec4.ai"><img src="BWS4-white-100.png" align="right" width="100" alt="Built with Spec4"></a>
...
Nine examples ship today. The first four are listed in order of how much machinery each pattern needs — **embeddings** (turn text into a position and compare positions), **single-call** (one prompt in, one response out), **RAG** (retrieve first, then answer from what you retrieved), and **tool use** (let the model decide what to call). Four more are appended after them, each adding a way of composing calls rather than a new kind of call: **chained calls** (one call's output becomes the next call's input), **planning agent** (one call decomposes a goal into a plan you approve before any step runs, then executes the steps and composes an itinerary), **orchestrated subagents** (a coordinator picks two of four fixed specialists, briefs each differently, runs them at the same time, and merges their independent answers), **multi-agent collaboration** (a buyer agent negotiates with two rival sellers that hold private constraints neither can see, across a staged trust boundary), and **the ReAct loop** (think, act, observe, then decide the next step from what came back — the same tier as the planning agent and its deliberate opposite). Seeing them side by side is the point: each one exists partly to show what the cheaper tier below it cannot do.
...
- **ReAct-loop example app** — the interleaved counterpart to the planning agent, in the same pattern tier: no plan is shown up front and you approve nothing mid-run. Each cycle the model produces a thought, chooses either a search or to answer, and reads the observation before deciding anything else — so the second query is written only after the first result has been read, which is what makes a multi-hop question answerable at all. Observations are rendered **verbatim** from the search results, so a fabricated fact is visibly absent from the evidence rather than blended into it, and a near-duplicate query is refused before it reaches the provider. Runs end in exactly one of two ways and both are shown plainly: a final answer naming the observations it drew on, or an honest budget-exhausted card saying what stayed unresolved. Afterwards each hop is labelled *observed* or *from memory*, and every grounding claim is re-derived from the trace rather than taken from the model's word for it.
...
- Exa (web search API), exposed as a shared framework capability in `backend/app/services/web_search.py` and used today by the tool-use example app and the ReAct loop
...
| `REACT_CYCLE_BUDGET` | — | Search cycles one ReAct run may take (default 8). **The dial for the gallery's most expensive example**: a run reserves its whole worst case — this many cycles plus a final-answer call plus a post-run annotation call, so 10 at the default — before its first cycle, and refunds whatever it does not spend. Lowering it lowers both the reservation and the worst-case search spend, at the cost of making the budget-exhausted ending more common. Not visitor-settable: the reservation is sized from this, so a client-supplied budget could reserve one number and spend another. |
| `REACT_DUPLICATE_SIMILARITY_THRESHOLD` | — | How close a new query may be to one already issued in the same run before it is refused (default 0.95). Deliberately high: refusing a genuinely *distinct* next hop costs the loop a step it cannot recover, while letting one near-duplicate through costs one search of eight. Measured — a synonym swap scores 0.976 and is blocked, a heavy rewording scores 0.714 and is allowed. |
| `REACT_SUITABILITY_TIMEOUT_SECONDS` | — | Hard timeout on the advisory "will this question exercise the loop?" check (default 6). It fails open: a timeout means no hint and Start stays enabled. Against a free-tier chain this fires on a substantial fraction of cold calls, so expect the hint to be absent about as often as it is present. |
| `REACT_SUITABILITY_CHECKS_PER_SESSION` | — | Cap on advisory checks per browser session (default 5). Beyond it the neutral state is served without a call. |
| `REACT_SUITABILITY_CACHE_TTL_HOURS` | — | How long a verdict is cached against the SHA-256 of the normalised question (default 24). The question text itself is never stored — only the hash and the derived fields. |
